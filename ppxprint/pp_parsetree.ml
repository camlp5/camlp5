...
module Location =
  struct
    ...
    [@@@"end"];
    type loc 'a =
      Location.loc 'a == { txt : 'a; loc : t }[@@"deriving_inline" show;]
    ;
    value rec pp_loc : ! 'a . Fmt.t 'a -> Fmt.t (loc 'a) =
      fun (type a) (tp_0 : Fmt.t a) (ofmt : Format.formatter) arg ->
        (fun ofmt ({txt = v_txt; loc = v_loc} : loc a) ->
           ...
           pf ofmt
             ...
             tp_0 v_txt pp v_loc)
          ofmt arg[@@"ocaml.warning" "-39";] [@@"ocaml.warning" "-33";]
    and show_loc : ! 'a . Fmt.t 'a -> loc 'a -> Stdlib.String.t =
...
module Asttypes =
  struct
    type loc 'a =
      Location.loc 'a == { txt : 'a; loc : Location.t }[@@"deriving_inline" show;]
    ;
    value rec pp_loc : ! 'a . Fmt.t 'a -> Fmt.t (loc 'a) =
      fun (type a) (tp_0 : Fmt.t a) (ofmt : Format.formatter) arg ->
        (fun ofmt ({txt = v_txt; loc = v_loc} : loc a) ->
...
and pattern_desc =
  Parsetree.pattern_desc ==
    ...
    | Ppat_construct of Asttypes.loc Longident.t and option pattern
...
and pp_expression_desc : Fmt.t expression_desc =
  fun (ofmt : Format.formatter) arg ->
    (fun ofmt ->
       ...
       | Pexp_function v0 ->
           let open Ppxprint_runtime.Runtime.Fmt in
           pf ofmt "(@[<2>Parsetree.Pexp_function@ %a)@]"
             (fun (ofmt : Format.formatter) arg ->
                let open Ppxprint_runtime.Runtime.Fmt in
                pf ofmt "@[<2>[%a@,]@]" (list ~{sep = semi} pp_case) arg)
             v0
       | Pexp_fun v0 v1 v2 v3 ->
           let open Ppxprint_runtime.Runtime.Fmt in
           pf ofmt "(@[<2>Parsetree.Pexp_fun@ (@,%a,@ %a,@ %a,@ %a@,))@]"
             ...
             (fun ofmt ->
                ...
                | Some arg ->
                    let open Ppxprint_runtime.Runtime.Fmt in
                    pf ofmt "(Some %a)" pp_expression arg ])
             v1 pp_pattern v2 pp_expression v3
       | Pexp_apply v0 v1 ->
           let open Ppxprint_runtime.Runtime.Fmt in
...
and pp_constructor_declaration : Fmt.t constructor_declaration =
  fun (ofmt : Format.formatter) arg ->
    (fun ofmt
         ({pcd_name = v_pcd_name; pcd_args = v_pcd_args; pcd_res = v_pcd_res;
           pcd_loc = v_pcd_loc; pcd_attributes = v_pcd_attributes} :
          constructor_declaration) ->
       let open Ppxprint_runtime.Runtime.Fmt in
       pf ofmt
         "@[<2>{ @[Parsetree.pcd_name =@ %a@];@ @[pcd_args =@ %a@];@ @[pcd_res =@ %a@];@ @[pcd_loc =@ %a@];@ @[pcd_attributes =@ %a@] }@]"
         (Asttypes.pp_loc
            (fun ofmt arg ->
               let open Ppxprint_runtime.Runtime.Fmt in pf ofmt "%S" arg))
         v_pcd_name pp_constructor_arguments v_pcd_args
         (fun ofmt ->
            fun
            [ None ->
                let open Ppxprint_runtime.Runtime.Fmt in
...
and pp_extension_constructor_kind : Fmt.t extension_constructor_kind =
  fun (ofmt : Format.formatter) arg ->
    (fun ofmt ->
       ...
       [ Pext_decl v0 v1 ->
           let open Ppxprint_runtime.Runtime.Fmt in
           pf ofmt "(@[<2>Parsetree.Pext_decl@ (@,%a,@ %a@,))@]"
             pp_constructor_arguments v0
             (fun ofmt ->
                fun
                [ None ->
                    let open Ppxprint_runtime.Runtime.Fmt in
...
and pp_class_infos : ! 'a . Fmt.t 'a -> Fmt.t (class_infos 'a) =
  fun (type a) (tp_0 : Fmt.t a) (ofmt : Format.formatter) arg ->
    (fun ofmt
       ...
       pf ofmt
         ...
         v_pci_attributes)
      ofmt arg[@@"ocaml.warning" "-39";] [@@"ocaml.warning" "-33";]
and show_class_infos : ! 'a . Fmt.t 'a -> class_infos 'a -> Stdlib.String.t =
  fun (type a) (tp_0 : Fmt.t a) arg ->
...
and pp_signature_item : Fmt.t signature_item =
  fun (ofmt : Format.formatter) arg ->
    (fun ofmt
         ({psig_desc = v_psig_desc; psig_loc = v_psig_loc} :
          signature_item) ->
       let open Ppxprint_runtime.Runtime.Fmt in
...
and pp_open_infos : ! 'a . Fmt.t 'a -> Fmt.t (open_infos 'a) =
  fun (type a) (tp_0 : Fmt.t a) (ofmt : Format.formatter) arg ->
    (fun ofmt
      ...
      ofmt arg[@@"ocaml.warning" "-39";] [@@"ocaml.warning" "-33";]
and show_open_infos : ! 'a . Fmt.t 'a -> open_infos 'a -> Stdlib.String.t =
  fun (type a) (tp_0 : Fmt.t a) arg ->
...
and pp_include_infos : ! 'a . Fmt.t 'a -> Fmt.t (include_infos 'a) =
  fun (type a) (tp_0 : Fmt.t a) (ofmt : Format.formatter) arg ->
    (fun ofmt
       ...
       pf ofmt
         ...
         v_pincl_attributes)
      ofmt arg[@@"ocaml.warning" "-39";] [@@"ocaml.warning" "-33";]
and show_include_infos : ! 'a . Fmt.t 'a -> include_infos 'a -> Stdlib.String.t =
  fun (type a) (tp_0 : Fmt.t a) arg ->
...
and pp_include_description : Fmt.t include_description =
  fun (ofmt : Format.formatter) arg ->
    pp_include_infos pp_module_type ofmt arg[@@"ocaml.warning" "-39";] [@@"ocaml.warning" "-33";]
...
and pp_include_declaration : Fmt.t include_declaration =
  fun (ofmt : Format.formatter) arg ->
    pp_include_infos pp_module_expr ofmt arg[@@"ocaml.warning" "-39";] [@@"ocaml.warning" "-33";]
...
and pp_structure_item : Fmt.t structure_item =
  fun (ofmt : Format.formatter) arg ->
    (fun ofmt
         ({pstr_desc = v_pstr_desc; pstr_loc = v_pstr_loc} :
          structure_item) ->
       let open Ppxprint_runtime.Runtime.Fmt in
...
and pp_value_binding : Fmt.t value_binding =
  fun (ofmt : Format.formatter) arg ->
    (fun ofmt
       ...
       let open Ppxprint_runtime.Runtime.Fmt in
       pf ofmt
         "@[<2>{ @[Parsetree.pvb_pat =@ %a@];@ @[pvb_expr =@ %a@];@ @[pvb_attributes =@ %a@];@ @[pvb_loc =@ %a@] }@]"
         pp_pattern v_pvb_pat pp_expression v_pvb_expr pp_attributes
         v_pvb_attributes Location.pp v_pvb_loc)
      ofmt arg[@@"ocaml.warning" "-39";] [@@"ocaml.warning" "-33";]
