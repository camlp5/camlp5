...
module type PARSE_RP_SIG = sig
  module Base : Mlsyntax.PARSEBASESIG ;
end
;

module PA(Base : Mlsyntax.PARSEBASESIG) : (PARSE_RP_SIG with module Base = Base) = struct
module Base = Base ;
open Base.Parsers ;
open Base;
...
EXTEND
  ...
  stream_patt_comp:
    [ [ "`"; p = patt; eo = V (OPT [ "when"; e = expr -> e ]) ->
          SpTrm loc p eo
      | "?="; pll = LIST1 lookahead SEP "|"; eo = V (OPT [ "when"; e = expr -> e ]) -> SpLhd loc pll eo
