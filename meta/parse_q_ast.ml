...
module Meta_make (C : MetaSig) =
  struct
    ...
    and patt =
      ...
      | PaTup _ lp clflag → C.node "PaTup" [C.vala (C.list patt) lp; C.vala C.bool clflag]
    ...
    and expr =
      ...
      | ExStr _ s →
          C.node "ExStr" [C.vala (fun (n,s) -> (C.tuple [C.loc_v(); C.vala C.string s])) s]
      | ExTry _ e lpoee →
          C.node "ExTry"
            [expr e;
             C.vala
...
value attribute_body_eoi = Grammar.Entry.create Base.Parsers.gram "attribute_body_eoi";
value class_expr_eoi = Grammar.Entry.create Base.Parsers.gram "class_expr";
value class_sig_item_eoi = Grammar.Entry.create Base.Parsers.gram "class_sig_item";
value class_str_item_eoi = Grammar.Entry.create Base.Parsers.gram "class_str_item";
value class_type_eoi = Grammar.Entry.create Base.Parsers.gram "class_type";
value ctyp_eoi = Grammar.Entry.create Base.Parsers.gram "type";
value expr_eoi = Grammar.Entry.create Base.Parsers.gram "expr";
value extended_longident_eoi = Grammar.Entry.create Base.Parsers.gram "extended_longident_eoi";
value extension_constructor_eoi = Grammar.Entry.create Base.Parsers.gram "extension_constructor_eoi";
value constructor_eoi = Grammar.Entry.create Base.Parsers.gram "constructor_eoi";
value longident_eoi = Grammar.Entry.create Base.Parsers.gram "longident_eoi";
value module_expr_eoi = Grammar.Entry.create Base.Parsers.gram "module_expr";
value module_type_eoi = Grammar.Entry.create Base.Parsers.gram "module_type";
value patt_eoi = Grammar.Entry.create Base.Parsers.gram "patt";
value poly_variant_eoi = Grammar.Entry.create Base.Parsers.gram "poly_variant";
value sig_item_eoi = Grammar.Entry.create Base.Parsers.gram "sig_item";
value str_item_eoi = Grammar.Entry.create Base.Parsers.gram "str_item";
value type_decl_eoi = Grammar.Entry.create Base.Parsers.gram "type_declaration";
value type_extension_eoi = Grammar.Entry.create Base.Parsers.gram "type_extension_eoi";
value with_constr_eoi = Grammar.Entry.create Base.Parsers.gram "with_constr";

EXTEND
  attribute_body_eoi: [ [ x = Base.Parsers.attribute_body; EOI -> x ] ];
  class_expr_eoi: [ [ x = Base.Parsers.class_expr ; EOI -> x ] ];
  class_sig_item_eoi: [ [ x = Base.Parsers.class_sig_item; EOI -> x ] ];
  class_str_item_eoi: [ [ x = Base.Parsers.class_str_item; EOI -> x ] ];
  class_type_eoi: [ [ x = Base.Parsers.class_type; EOI -> x ] ];
  ctyp_eoi: [ [ x = Base.Parsers.ctyp; EOI -> x ] ];
  expr_eoi: [ [ x = Base.Parsers.expr; EOI -> x ] ];
  extended_longident_eoi: [ [ x = Base.Parsers.extended_longident; EOI -> x ] ];
  extension_constructor_eoi: [ [ x = Base.Parsers.extension_constructor; EOI -> x ] ];
  constructor_eoi: [ [ x = Base.Parsers.constructor_declaration; EOI -> x ] ];
  longident_eoi: [ [ x = Base.Parsers.longident; EOI -> x ] ];
  module_expr_eoi: [ [ x = Base.Parsers.module_expr; EOI -> x ] ];
  module_type_eoi: [ [ x = Base.Parsers.module_type; EOI -> x ] ];
  patt_eoi: [ [ x = Base.Parsers.patt; EOI -> x ] ];
  poly_variant_eoi: [ [ x = Base.Parsers.poly_variant; EOI -> x ] ];
  sig_item_eoi: [ [ x = Base.Parsers.sig_item; EOI -> x ] ];
  str_item_eoi: [ [ x = Base.Parsers.str_item ; EOI -> x ] ];
  type_decl_eoi: [ [ x = Base.Parsers.type_decl; EOI -> x ] ];
  type_extension_eoi: [ [ x = Base.Parsers.type_extension; EOI -> x ] ];
  with_constr_eoi: [ [ x = Base.Parsers.with_constr; EOI -> x ] ];
END;
