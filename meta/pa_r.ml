...
EXTEND
  ...
  str_item:
    [ "top"
      ...
      | "exception"; check_exception_decl ; (_, c, tl, _) = constructor_declaration_sans_alg_attrs ; alg_attrs = alg_attributes ; item_attrs = item_attributes →
...
*)
          {MLast.teNam=n; tePrm=tpl; tePrv=pf; teDef=tk; teAttributes=attrs}
      ] ]
  ;
  mod_ident_patt:
    [ [ n = V mod_ident → (loc, n) ] ]
