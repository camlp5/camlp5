...
module type PARSE_Q_AST_BASE_SIG = sig
  module Base : Mlsyntax.PARSEBASESIG ;
  module QH : Quotation.QUOTATION_EXPANSION with module Base = Base ;

  value eval_anti : Grammar.Entry.e α → Ploc.t → string → string → (Ploc.t * α);
  value skip_to_next_colon : string → int → (int * int);
  value split_anti_loc : string → option (string * string * string);
  value replace_antiloc_kind : ~newkind:string → string → string;
  value get_anti_loc : string → option (Ploc.t * string * string);
  module type MetaSig = sig
    type t = α;
    type prefix_t = α;
    value loc_v : unit → t;
    value node : ?prefix:prefix_t → string → list t → t;
    value node_no_loc : ?prefix:prefix_t → string → list t → t;
    value list : (α → t) → list α → t;
    value option : (α → t) → option α → t;
    value vala : (α → t) → MLast.v α → t;
    value char : char → t;
    value bool : bool → t;
    value int : int → t;
    value int32 : int32 → t;
  ...
  value anti_anti : string → string;
  value is_anti_anti : string → bool;
  module E_MetaSig : sig
    type t = MLast.expr;
    type prefix_t = MLast.longid;
    value loc : Ploc.t;
    value loc_v : unit → MLast.expr;
    value node : ?prefix:MLast.longid → string → list MLast.expr → MLast.expr;
    value node_no_loc :
    ...
    value int32 : int32 → MLast.expr;
    value int64 : int64 → MLast.expr;
    value nativeint : nativeint → MLast.expr;
    value float : float → MLast.expr;
    value string : string → MLast.expr;
    value tuple : list MLast.expr → MLast.expr;
    value record : list (MLast.patt * MLast.expr) → MLast.expr;
    value xtr_typed : string → Ploc.t → string → MLast.expr;
    value xtr : Ploc.t → string → MLast.expr;
    value xtr_or_anti :
      Ploc.t → (MLast.expr → MLast.expr) → string → MLast.expr;
  end ;
  module P_MetaSig : sig
    type t = MLast.patt;
    type prefix_t = MLast.longid;
    value loc : Ploc.t;
    value loc_v : unit → MLast.patt;
    value node : ?prefix:MLast.longid → string → list MLast.patt → MLast.patt;
    value node_no_loc :
    ...
    value int32 : int32 → MLast.patt;
    value int64 : int64 → MLast.patt;
    value nativeint : nativeint → MLast.patt;
    value float : float → MLast.patt;
    value string : string → MLast.patt;
    value tuple : list MLast.patt → MLast.patt;
    value record : list (MLast.patt * MLast.patt) → MLast.patt;
    value xtr_typed : string → Ploc.t → string → MLast.patt;
    value xtr : Ploc.t → string → MLast.patt;
    value xtr_or_anti : Ploc.t → (MLast.patt → MLast.patt) → string → MLast.patt;
  end ;
  value check_anti_loc : string → string;
end
;

module PA(Base : Mlsyntax.PARSEBASESIG)
         (QH : Quotation.QUOTATION_EXPANSION with module Base = Base)
       : (PARSE_Q_AST_BASE_SIG with module Base = Base and module QH = QH)
  = struct
module Base = Base ;
module QH = QH ;
open Base.Parsers ;
...
value eval_anti entry loc typ str =
  ...
  let r =
    try
      Ploc.call_with Base.Lexer.force_antiquot_loc False
...
module E_MetaSig = struct
  ...
  value vala elem =
    ...
    ELSE
      ...
      [ Ploc.VaAnt s ->
          ...
          [ Some (loc, typ, str) ->
              let (loc, r) = eval_anti QH.expr_eoi loc typ str in
  ...
  value xtr_typed wantty loc s =
    ...
    [ Some (_, typ, str) when typ = wantty ->
        let (loc, r) = eval_anti QH.expr_eoi loc "" str in
        <:expr< $anti:r$ >>
    | _ -> assert False ]
  ;
  value xtr loc s =
  ...
  value xtr_or_anti loc f s =
    ...
    [ Some (_, typ, str) ->
        ...
        [ "" | "exp" ->
            ...
            <:expr< $anti:r$ >>
        | "anti" ->
            let (loc, r) = eval_anti QH.expr_eoi loc "anti" str in
            f <:expr< $anti:r$ >>
        | _ -> assert False ]
    | None -> assert False ]
  ;
...
module P_MetaSig = struct
  ...
  value vala elem =
    ...
    ELSE
      ...
      [ Ploc.VaAnt s ->
          match get_anti_loc s with
          [ Some (loc, typ, str) ->
              let (loc, r) = eval_anti QH.patt_eoi loc typ str in
  ...
  value xtr_typed wantty loc s =
    match get_anti_loc s with
    [ Some (_, typ, str) when typ = wantty ->
        let (loc, r) = eval_anti QH.patt_eoi loc "" str in
        <:patt< $anti:r$ >>
    | _ -> assert False ]
  ;
  value xtr loc s =
  ...
  value xtr_or_anti loc f s =
    ...
    [ Some (_, typ, str) ->
        match typ with
        [ "" | "exp" ->
            let (loc, r) = eval_anti QH.patt_eoi loc "exp" str in
            <:patt< $anti:r$ >>
        | "anti" ->
            let (loc, r) = eval_anti QH.patt_eoi loc "anti" str in
            f <:patt< $anti:r$ >>
        | _ -> assert False ]
    | None -> assert False ]
  ;
...
let lex = Grammar.glexer Base.Parsers.gram in
...
Grammar.iter_entry Grammar.reinit_entry_functions
  (Grammar.Entry.obj Base.Parsers.expr);
end
;
