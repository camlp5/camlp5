; camlp5 ./pa_schemer.cmo pa_extend.cmo parse_q_MLast.cmo q_MLast.cmo pr_dump.cmo
; pa_scheme.ml,v
   ...
   ((Sacc loc se1 se2)
    (let ((li1 (longid_se se1)))
      (match (longid_lident_se se2)
               ((values (Some <:vala< li2 >>) s) (values (Some <:vala< (longid_concat li1 li2) >>) s))
               ((values None s) (values (Some <:vala< li1 >>) s))
       ...
       (lambda_match
        ...
        ([se . sel]
         (let* ((e (expr_se se)) (el (loop sel)))
          <:expr< [$e$ :: $el$] >>)))))
     (loop sel)))
   ((Squot loc typ txt) (Pcaml.QH.handle_expr_quotation loc (values typ txt)))
       ...
       (lambda_match
        ...
        ([se . sel]
         (let* ((p (patt_se se)) (pl (loop sel)))
          <:patt< [$p$ :: $pl$] >>)))))
     (loop sel)))
   ((Squot loc typ txt) (Pcaml.QH.handle_patt_quotation loc (values typ txt)))
  ...
  (match se
   ((when (Slid _ s) (&& (>= (String.length s) 2) (= s.[0] ''')))
    (let ((s (String.sub s 1 (- (String.length s) 1))))
     (values <:vala< (Some s) >> <:vala< "" >>)))
   ((when (Slid _ s) (&& (>= (String.length s) 3) (= s.[1] ''')))
    (let
     ((vara
       (if (= s.[0] '+') <:vala< "+" >>
        (if (= s.[0] '-') <:vala< "-" >> (error se "type_param"))))
 ...
 (ctyp_se
  (lambda_match
   ...
   ((Sexpr loc [(Slid _ "*") . sel])
    (let* ((tl (anti_list_map ctyp_se sel))
          (tl (Pcaml.vala_map (List.map (lambda (ct) (values <:vala< None >> ct))) tl))
         )
         <:ctyp< ($_list:tl$) >>))
   ((Sexpr loc [(Slid _ "==") se1 se2])
