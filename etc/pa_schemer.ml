(* camlp5 parse_r.cmo pa_r.cmo parse_rp.cmo pa_rp.cmo pa_extend.cmo parse_q_MLast.cmo q_MLast.cmo pr_dump.cmo *)
(* pa_scheme.ml,v *)
...
and dollar bp kwt strm =
  if Pcaml.Lexer.force_antiquot_loc.val then
...
and constructor_declaration_se =
  ...
  [ Sexpr loc [Suid _ ci :: sel] →
      (loc, <:vala< (rename_id ci) >>, <:vala< [] >>, <:vala< (List.map ctyp_se sel) >>,
       <:vala< None >>, <:vala< [] >>)
