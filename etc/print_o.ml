...
value longident_lident pc (lio, id) =
  match lio with
  [ None -> pprintf pc "%p" var_escaped_noloc (uv id)
  | Some li -> pprintf pc "%p.%p" longident (uv li) var_escaped_noloc (uv id)
  ]
;
...
value type_param pc (loc, (tv, vastr)) =
  let tv = uv tv in
...
value type_decl pc td =
  ...
  [ <:ctyp:< '$s$ >> when not (mem_tvar s (uv tp))
                          && not (List.exists (fun [ (t1, t2) -> Ast2pt.ctyp_mentions s t1 || Ast2pt.ctyp_mentions s t2 ]) (uv cl)) ->
      pprintf pc "%p%p%p%p" type_params (loc, uv tp)
        var_escaped (loc, uv tn)
        ...
        horiz_vertic
          ...
          (fun () ->
             pprintf pc "@[<a>%p%p %s@;%s%p%p%p@ @]"
               type_params
               (loc, uv tp) var_escaped (loc, uv tn)
               asgn
               (if pf then "private " else "")
               ctyp
               te (hlist type_constraint) (uv cl)
               (hlist (pr_attribute "@@")) (uv attrs)) ]
;

value label_decl pc (_, l, m, t, attrs) =
  pprintf pc "%s%p :@;%p%p" (if m then "mutable " else "") var_escaped_noloc l ctyp_below_alg_attribute t
  (hlist (pr_attribute "@")) (uv attrs)
;
  ...
  if tl = [] then do {
    ...
    | (_, None) ->
        pprintf pc "%p of@;<1 4>%p%p" cons_escaped (loc, c) (plist ctyp_apply 2)
          tl (hlist (pr_attribute "@")) (uv alg_attrs) ]
  }
]
;
...
EXTEND_PRINTER
  ...
  pr_patt:
    ...
    | "atomic"
      ...
      | <:patt< (type $lid:s$) >> ->
          pprintf pc "(type %p)" var_escaped_noloc s
      ...
      [ <:ctyp< ($list:tl$) >> ->
          ...
          let pp1 pc (lab, ct) =
            match uv lab with [
              ...
              | Some l -> pprintf pc "%s:%p" (uv l) next ct
              ] in
          plist pp1 2 pc tl ]
   | "apply"
      [ <:ctyp:< $t1$ $t2$ >> ->
  ...
  pr_str_item:
    [ "top"
      ...
      | <:str_item< module type $m$ = ' $id$ $_itemattrs:item_attrs$ >> ->
          pprintf pc "module type %s%p" m (hlist (pr_attribute "@@")) (uv item_attrs)
...
value class_def pc ci =
  pprintf pc "%s%p%p :@;%p%p"
    (if uv ci.MLast.ciVir then "virtual " else "")
    class_type_params (ci.MLast.ciLoc, uv (snd ci.MLast.ciPrm))
    var_escaped_noloc (uv ci.MLast.ciNam) class_type ci.MLast.ciExp
    (hlist (pr_attribute "@@")) (uv ci.MLast.ciAttributes)
;
    ...
    horiz_vertic
      (fun () ->
         pprintf pc "%s%p%p%s%p%p ="
           (if uv ci.MLast.ciVir then "virtual " else "")
           class_type_params (ci.MLast.ciLoc, uv (snd ci.MLast.ciPrm))
           var_escaped_noloc (uv ci.MLast.ciNam) (if pl = [] then "" else " ")
            ...
            bef =
              sprintf "%s%s%s%s " pc.bef
                ...
                (class_type_params Pprintf.empty_pc
                   (ci.MLast.ciLoc, uv (snd ci.MLast.ciPrm)))
                (var_escaped_bare (uv ci.MLast.ciNam))}
         in
...
value variant_decl pc pv =
  ...
  [ <:poly_variant:< `$c$ $_algattrs:alg_attrs$ >> ->
       pprintf pc "`%p%p" var_escaped (loc, c) (hlist (pr_attribute "@")) (uv alg_attrs)
  | <:poly_variant:< `$c$ of $flag:ao$ $list:tl$ $_algattrs:alg_attrs$ >> ->
       let tl = List.map (fun t -> (t, " &")) tl in
       pprintf pc "`%p of%s@;<1 5>%p%p" var_escaped (loc, c) (if ao then " &" else "")
         (plist ctyp 2) tl (hlist (pr_attribute "@")) (uv alg_attrs)
  | <:poly_variant< $t$ >> ->
       ctyp pc t
  | IFDEF STRICT THEN
...
value typevar pc tv = pprintf pc "'%p" var_escaped_noloc tv;
...
value sig_method_or_method_virtual pc virt priv s t item_attrs =
  pprintf pc "method%s%s %p :@;%p%p" virt (if priv then " private" else "")
    var_escaped_noloc s ctyp t (hlist (pr_attribute "@@")) (uv item_attrs)
  ...
  pr_class_expr:
    [ "top"
      ...
      | <:class_expr< let open $_!:ovf$ $longid:li$ in $ce$ >> ->
          let ovf = if (uv ovf) then "!" else "" in
          if pc.dang = ";" then
    ...
    [ "top"
      [ <:class_str_item< inherit $!:ovf$ $ce$ $opt:pb$ $_itemattrs:item_attrs$ >> ->
          pprintf pc "inherit%s@;%p@[%p%p@]" (if ovf then "!" else "") class_expr ce
            (fun pc ->
               fun
               [ Some s -> pprintf pc " as %p" var_escaped_noloc s
               | None -> pprintf pc "" ])
            pb
            (hlist (pr_attribute "@@")) (uv item_attrs)
      ...
      | <:class_str_item<
        ...
        >> ->
          ...
          match topt with
          [ None ->
              pprintf pc "method%s%s %p%s%p =@;%p%p"
                (if ov then "!" else "") (if priv then " private" else "") var_escaped_noloc s
                (if pl = [] then "" else " ") (hlist simple_patt) pl
                expr e (hlist (pr_attribute "@@")) (uv item_attrs)
          | Some t ->
              pprintf pc "method%s%s %p%s%p :@;<1 4>%p =@;%p%p"
                (if ov then "!" else "") (if priv then " private" else "") var_escaped_noloc s
                (if pl = [] then "" else " ") (hlist simple_patt) pl
                poly_type t expr e (hlist (pr_attribute "@@")) (uv item_attrs) ]
      | <:class_str_item< type $t1$ = $t2$ $_itemattrs:item_attrs$ >> ->
          pprintf pc "constraint %p =@;%p%p" ctyp t1 ctyp t2
            (hlist (pr_attribute "@@")) (uv item_attrs)
      | <:class_str_item< value $!:ovf$ $flag:mf$ $lid:s$ = $e$ $_itemattrs:item_attrs$ >> ->
          pprintf pc "val%s%s %p =@;%p%p" (if ovf then "!" else "")
            (if mf then " mutable" else "") var_escaped_noloc s expr e
            (hlist (pr_attribute "@@")) (uv item_attrs)
      | <:class_str_item< value virtual $flag:mf$ $lid:s$ : $t$ $_itemattrs:item_attrs$ >> ->
          pprintf pc "val virtual%s %p :@;%p%p"
            (if mf then " mutable" else "") var_escaped_noloc s ctyp t
            (hlist (pr_attribute "@@")) (uv item_attrs)
