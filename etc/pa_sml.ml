    ...
    List.fold_right
      (fun (loc, c, tl, aux_opt, alg_attrs) (cdl, aux) -> do {
         ...
         [ Some anon_record_type ->
             ...
             ([(loc, <:vala< c >>, <:vala< [] >>, <:vala< tl >>, <:vala< None >>, <:vala< [] >>) :: cdl],
                [aux_def :: aux])
         | None -> ([(loc, <:vala< c >>, <:vala< [] >>, <:vala< tl >>, <:vala< None >>, <:vala< [] >>) :: cdl], aux) ]
         })
      cdol ([], [])
  in
  let td1 =
...
EXTEND
  ...
  tyvar_pc:
    [ [ "'"; x1 = V tyvar -> [(x1, <:vala< "" >>)]
      | "'"; x1 = V tyvar; ","; l = tyvar_pc -> [(x1, <:vala< "" >>) :: l] ] ]
  ;
  tyvar:
    [ [ v = LIDENT -> Some v ] ]
  ;
  ...
  ;
  eqid:
    [ [ x1 = UIDENT; "."; x2 = eqid -> Asttools.expr_concat <:expr< $uid:x1$ >> x2
  ...
  tb:
    [ [ x1 = tyvars; x2 = idd; "="; x3 = ctyp ->
      ...
      | x1 = tyvars; x2 = idd; "="; x3 = ctyp; "=="; x4 = dbrhs ->
          let x4 =
            List.map
              (fun (loc, c, tl, _, alg_attrs) -> (loc, <:vala< c>>, <:vala< [] >>, <:vala< tl >>, <:vala< None >>, alg_attrs))
              x4
          in
          {MLast.tdIsDecl = <:vala< True >> ;
           MLast.tdNam = <:vala< (loc, <:vala< uncap x2 >>) >>;
          ...
          let x4 =
            if list = [] then x4
              ...
              [ <:module_expr< struct $list:list$ end >> ->
                  let si = let loc = Ploc.dummy in <:str_item< open AAA >> in
