...
value type_binder_delta = [
    ("START",fun [
                 ("","'") -> "QUO"
               | ("GIDENT",_) -> "IDS"
               | ("ANTIQUOT", s) when Some "list" = (s |> Plexing.parse_antiquot |> option_map fst) -> "PREDOT"
               | ("ANTIQUOT", s) when Some "_list" = (s |> Plexing.parse_antiquot |> option_map fst) -> "PREDOT"
               | ("ANTIQUOT_LOC", s) when Some "list" = (s |> Plexing.parse_antiloc |> option_map snd3) -> "PREDOT"
               | ("ANTIQUOT_LOC", s) when Some "_list" = (s |> Plexing.parse_antiloc |> option_map snd3) -> "PREDOT"
               | _ -> failwith "START"
    ])
   ;("PREDOT",fun [
               ("",".") -> "ACCEPT"
...
value split_rawstring s =
  try
    let i = String.index s ':' in
    let delimsize_s = String.sub s 0 i in
    let rest = String.sub s (i+1) (String.length s - i - 1) in
    let delimsize = int_of_string delimsize_s in
    (delimsize, rest)
  with 
  [ Not_found | Failure _ ->
    failwith (Printf.sprintf "Asttools.split_rawstring: unexpected malformed rawstring \"%s\"" s) ]
;
value narrow_loc loc left right =
  let open Ploc in
  let len = (last_pos loc) - (first_pos loc) in
  (sub loc left (len - left - right))
  ;
