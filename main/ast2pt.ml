...
value mkfield ~{alg_attributes=alg_attributes} loc d fl = ocaml_mkfield ~{alg_attributes=alg_attributes} (mkloc loc) d fl;
...
and meth_list loc fl v =
  ...
  | [(lab, t, attrs) :: fl] →
      mkfield ~{alg_attributes=uv_alg_attributes attrs} loc (lab, add_polytype t) (meth_list loc fl v) ]
and add_polytype t =
  match ocaml_ptyp_poly with
  [ Some ptyp_poly →
      match t with
...
and expr =
      ...
      if is_module_path e1 then
          ...
          mkexp loc
            (ocaml_pexp_apply
               ...
               [("", expr e1); ("", expr e2)])
  | ExAre loc _ e1 e2 → assert False
  ...
  | ExAss loc e v →
      ...
      | ExAre _ _ e1 e2 → assert False
      | ExBae loc (Ploc.VaVal ".") e el → expr (bigarray_set loc e (uv el) v)
      | ExBae loc _ e el → assert False
      | ExLid _ lab → mkexp loc (ocaml_pexp_setinstvar (uv lab) (expr v))
      | ExSte _ (Ploc.VaVal ".") e1 e2 →
          let cloc = mkloc loc in
          mkexp loc
            (ocaml_pexp_apply
               (mkexp loc
  ...
  | ExBae loc (Ploc.VaVal ".") e el → expr (bigarray_get loc e (uv el))
  | ExBae loc _ e el → assert False
  | ExChr loc s →
      mkexp loc
...
and str_item s l =
  ...
  | StMty loc n mt item_attrs →
      let m = ocaml_pstr_modtype ~{item_attributes=uv_item_attributes item_attrs} (mkloc loc) (uv n) (module_type mt) in
      [mkstr loc m :: l]
  | StMtyAbs loc n item_attrs →
      let m = ocaml_pstr_modtype_abs ~{item_attributes=uv_item_attributes item_attrs} (mkloc loc) (uv n) in
      [mkstr loc m :: l]
