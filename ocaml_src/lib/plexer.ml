...
module Make (_ : sig  end) =
  struct
    ...
    let number buf (strm__ : _ Stream.t) =
      ...
      match Stream.peek strm__ with
        Some '.' ->
          Stream.junk strm__;
          let buf =
            decimal_digits_under (Plexing.Lexbuf.add '.' buf) strm__
          in
          begin match
    ...
    let hex_number buf (strm__ : _ Stream.t) =
      ...
      match Stream.peek strm__ with
        Some '.' ->
          Stream.junk strm__;
          let buf =
            hex_digits_under_star (Plexing.Lexbuf.add '.' buf) strm__
          in
          begin match
            begin try Some (hex_exponent_part buf strm__) with
              Stream.Failure -> None
            end
          with
            Some buf -> "FLOAT", Plexing.Lexbuf.get buf
      ...
      | _ ->
          match
            try Some (hex_exponent_part buf strm__) with
              Stream.Failure -> None
          with
            Some buf -> "FLOAT", Plexing.Lexbuf.get buf
          | _ ->
    ...
    let any ctx buf (strm__ : _ Stream.t) =
      ...
      match Stream.peek strm__ with
        Some c ->
          Stream.junk strm__; ctx.line_cnt bp c; Plexing.Lexbuf.add c buf
      | _ -> raise Stream.Failure
    ;;
    ...
    let rec quotation ctx bp buf (strm__ : _ Stream.t) =
      ...
      | Some '<' ->
          ...
          | Some ':' ->
              Stream.junk strm__;
              let buf =
                ident (Plexing.Lexbuf.add ':' (Plexing.Lexbuf.add '<' buf))
                  strm__
              in
              begin match Stream.peek strm__ with
                Some '<' ->
    ...
    let rec antiquot_rest ctx bp buf (strm__ : _ Stream.t) =
      ...
      | _ ->
          ...
          | _ ->
              err ctx (bp, Stream.count strm__) "antiquotation not terminated"
    ;;
    let rec antiquot ctx bp buf (strm__ : _ Stream.t) =
      ...
      | Some ('a'..'z' | 'A'..'Z' | '0'..'9' | '!' | '_' as c) ->
          Stream.junk strm__;
          antiquot ctx bp (Plexing.Lexbuf.add c buf) strm__
      | Some ':' ->
          Stream.junk strm__;
          let buf =
            antiquot_rest ctx bp (Plexing.Lexbuf.add ':' buf) strm__
          in
          Plexing.Lexbuf.get buf
      | Some '\\' ->
          Stream.junk strm__;
    ...
    let rec any_to_nl buf (strm__ : _ Stream.t) =
      match Stream.peek strm__ with
        Some ('\r' | '\n' as c) ->
          Stream.junk strm__; Plexing.Lexbuf.add c buf
      | Some c ->
          Stream.junk strm__; any_to_nl (Plexing.Lexbuf.add c buf) strm__
      | _ -> buf
    ;;
      ...
      match Stream.peek strm__ with
        Some '|' ->
          ...
          rawstring1 (Plexing.Lexbuf.get buf)
            (0, "|" ^ Plexing.Lexbuf.get buf ^ "}") ctx Plexing.Lexbuf.empty
            strm
      | Some ('a'..'z' | '_' as c) ->
          Stream.junk strm__;
          let strm = strm__ in
          rawstring0 ctx bp (Plexing.Lexbuf.add c buf) strm
      | _ -> raise Stream.Failure
    ;;
    ...
    let comment_quoted_extension ctx bp buf (strm__ : _ Stream.t) =
      ...
      try
        zerobuf (comment_quoted_extension0 ctx bp (Plexing.Lexbuf.get buf))
          buf strm__
      with Stream.Failure -> raise (Stream.Error "")
    ;;
    let comment ctx bp =
      let rec comment buf (strm__ : _ Stream.t) =
        ...
        | _ ->
            match
              try Some (any ctx buf strm__) with Stream.Failure -> None
            with
    ...
    ;;
    let quoted_extension (kind : string) ctx (bp, _) buf
        (strm__ : _ Stream.t) =
      let buf = extattrident buf strm__ in
    ...
    let rec dotsymbolchar_star buf (strm__ : _ Stream.t) =
      match
        try Some (dotsymbolchar buf strm__) with Stream.Failure -> None
      with
        Some buf ->
          ...
          | _ ->
              ...
              | _ ->
                  ...
                  | Some '0' ->
                      ...
                      begin match Stream.peek strm__ with
                        Some ('o' | 'O' as c) ->
                          Stream.junk strm__;
                          digits octal
                            (Plexing.Lexbuf.add c
                               (Plexing.Lexbuf.add '0' buf))
                            strm__
                      | Some ('x' | 'X' as c) ->
                          Stream.junk strm__;
                          hex_number
                  ...
                  | Some '!' ->
                      Stream.junk strm__;
                      let buf =
                        hash_follower_chars (Plexing.Lexbuf.add '!' buf)
                          strm__
                      in
                      keyword_or_error ctx (bp, Stream.count strm__)
                        (Plexing.Lexbuf.get buf)
                                ...
                                begin try
                                  match Stream.peek strm__ with
                                    Some ('a'..'z' as c) ->
                                      Stream.junk strm__;
                                      let buf =
                                        ident (Plexing.Lexbuf.add c buf)
                                          strm__
                                      in
                                  ...
                                  | Some '_' ->
                                      Stream.junk strm__;
                                      let buf =
                                        ident (Plexing.Lexbuf.add '_' buf)
                                          strm__
                                      in
                                      tildeident ~raw:true ctx
                                        (bp, Stream.count strm__) buf strm__
                        ...
                        | _ ->
                            ...
                            | Some '%' ->
                                ...
                                begin try
                                  ...
                                  | _ ->
                                      quoted_extension "QUOTEDEXTENSION_EXPR"
    ...
    let rec next_token ctx buf (strm__ : _ Stream.t) =
      ...
      | Some '(' ->
          ...
          | _ ->
              let ep = Stream.count strm__ in
              let loc =
                ctx.make_lined_loc (bp, ep) (Plexing.Lexbuf.get buf)
              in
      ...
      | _ ->
          ...
          try
            match
              try
                Some
                  (next_token_after_spaces ctx bp Plexing.Lexbuf.empty strm__)
              with Stream.Failure -> None
            with
              Some tok ->
                let ep = Stream.count strm__ in
    ...
    let make_ctx kwd_table =
      ...
      {after_space = false;
       ...
       set_line_nb =
         (fun () ->
            line_nb := !(!(Plexing.line_nb));
            bol_pos := !(!(Plexing.bol_pos)));
       make_lined_loc =
         fun loc comm ->
          ...
          match Stream.peek strm__ with
            Some
              ('!' | '?' | '~' | '=' | '@' | '^' | '&' | '+' | '-' | '*' |
               '/' | '%' | '.' as c) ->
              Stream.junk strm__;
              check_ident2 (Plexing.Lexbuf.add c buf) strm__
          | Some '$' ->
              Stream.junk strm__;
              ...
              function
                "ANTIQUOT", prm ->
                  if prm <> "" && prm.[String.length prm - 1] = ':' then
                    if eq_before_colon p_prm prm then
                      after_colon_except_last prm
                    else raise Stream.Failure
                  else raise Stream.Failure
              | _ -> raise Stream.Failure
            else
    ...
    let gmake () =
      ...
      let glexr =
        ref
          {Plexing.tok_func =
            (fun _ -> raise (Match_failure ("plexer.ml", 1152, 25)));
           tok_using =
             (fun _ -> raise (Match_failure ("plexer.ml", 1152, 45)));
           tok_removing =
             (fun _ -> raise (Match_failure ("plexer.ml", 1152, 68)));
           tok_match =
             (fun _ -> raise (Match_failure ("plexer.ml", 1153, 18)));
           tok_text =
             (fun _ -> raise (Match_failure ("plexer.ml", 1153, 37)));
