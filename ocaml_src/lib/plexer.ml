      ...
      Some c ->
        if Char.code c >= 128 then
          ...
          if List.mem x greek_tab then
            begin Istream.junk strm; Plexing.Lexbuf.add c buf end
...
let misc_letter buf strm =
  if !utf8_lexing then
    match Istream.peek strm with
      Some c ->
        if Char.code c >= 128 then
          ...
          | _ -> Istream.junk strm; Plexing.Lexbuf.add c buf
        else raise Istream.Failure
    | None -> raise Istream.Failure
  else
    ...
    match Istream.peek strm__ with
      Some ('\128'..'\225' as c) ->
        Istream.junk strm__; Plexing.Lexbuf.add c buf
    | Some ('\227'..'\255' as c) ->
        Istream.junk strm__; Plexing.Lexbuf.add c buf
...
let misc_punct buf strm =
  if !utf8_lexing then
    ...
    match Istream.peek strm__ with
      Some '\226' ->
        Istream.junk strm__;
        begin match Istream.peek strm__ with
          Some c ->
            Istream.junk strm__;
            begin match Istream.peek strm__ with
              Some c1 ->
            ...
            | _ -> raise (Istream.Error "")
            end
        | _ -> raise (Istream.Error "")
        end
    | _ -> raise Istream.Failure
  else let (_ : _ Istream.t) = strm in raise Istream.Failure
;;
          ...
          match Istream.peek strm__ with
            Some '\134' ->
              ...
              begin match Istream.peek strm__ with
                Some '\146' ->
                  Istream.junk strm__;
                  keyword_or_error ctx (bp, Istream.count strm__) "->"
              | _ -> raise (Istream.Error "")
              end
          | Some '\137' ->
              Istream.junk strm__;
...
let rec ident buf (strm__ : _ Istream.t) =
  match
    try
      Some
        (match Istream.peek strm__ with
    ...
    with Istream.Failure -> None
  with
    Some buf -> ident buf strm__
  | _ -> buf
;;

let rec ident2 buf (strm__ : _ Istream.t) =
  match
    try
      Some
        (match Istream.peek strm__ with
...
let rec ident3 buf (strm__ : _ Istream.t) =
  match Istream.peek strm__ with
    Some
      ('0'..'9' | 'A'..'Z' | 'a'..'z' | '_' | '!' | '%' | '&' | '*' | '+' |
       '-' | '.' | '/' | ':' | '<' | '=' | '>' | '?' | '@' | '^' | '|' | '~' |
       '\'' | '$' | '\128'..'\255' as c) ->
      Istream.junk strm__; ident3 (Plexing.Lexbuf.add c buf) strm__
  | _ -> buf
;;

let binary buf (strm__ : _ Istream.t) =
  match Istream.peek strm__ with
    Some ('0' | '1' as c) -> Istream.junk strm__; Plexing.Lexbuf.add c buf
  | _ -> raise Istream.Failure
;;
let octal buf (strm__ : _ Istream.t) =
  match Istream.peek strm__ with
    Some ('0'..'7' as c) -> Istream.junk strm__; Plexing.Lexbuf.add c buf
  | _ -> raise Istream.Failure
;;
let decimal buf (strm__ : _ Istream.t) =
  match Istream.peek strm__ with
    Some ('0'..'9' as c) -> Istream.junk strm__; Plexing.Lexbuf.add c buf
  | _ -> raise Istream.Failure
;;
let hexa buf (strm__ : _ Istream.t) =
  match Istream.peek strm__ with
...
let end_integer buf (strm__ : _ Istream.t) =
  match Istream.peek strm__ with
    Some 'l' -> Istream.junk strm__; "INT_l", Plexing.Lexbuf.get buf
  | Some 'L' -> Istream.junk strm__; "INT_L", Plexing.Lexbuf.get buf
  | Some 'n' -> Istream.junk strm__; "INT_n", Plexing.Lexbuf.get buf
  | _ -> "INT", Plexing.Lexbuf.get buf
;;

let rec digits_under kind buf (strm__ : _ Istream.t) =
  match try Some (kind buf strm__) with Istream.Failure -> None with
...
let digits kind buf (strm__ : _ Istream.t) =
  let buf =
    try kind buf strm__ with
      Istream.Failure -> raise (Istream.Error "ill-formed integer constant")
  in
  digits_under kind buf strm__
;;

let rec decimal_digits_under buf (strm__ : _ Istream.t) =
  match Istream.peek strm__ with
    Some ('0'..'9' | '_' as c) ->
      Istream.junk strm__;
...
let exponent_part buf (strm__ : _ Istream.t) =
  match Istream.peek strm__ with
    Some ('e' | 'E' as c) ->
      Istream.junk strm__;
      ...
      begin match Istream.peek strm__ with
        Some ('0'..'9' as c) ->
          Istream.junk strm__;
          decimal_digits_under (Plexing.Lexbuf.add c buf) strm__
      | _ -> raise (Istream.Error "ill-formed floating-point constant")
      end
  | _ -> raise Istream.Failure
;;

let number buf (strm__ : _ Istream.t) =
  ...
  match Istream.peek strm__ with
    Some '.' ->
      ...
      begin match
        (try Some (exponent_part buf strm__) with Istream.Failure -> None)
      with
...
let char_after_bslash buf (strm__ : _ Istream.t) =
  match Istream.peek strm__ with
    Some '\'' -> Istream.junk strm__; buf
  | Some c ->
      Istream.junk strm__;
      let buf = Plexing.Lexbuf.add c buf in
      begin try
...
let char ctx bp buf (strm__ : _ Istream.t) =
  match Istream.peek strm__ with
    Some '\\' ->
      Istream.junk strm__;
      begin match Istream.peek strm__ with
      ...
      | _ -> err ctx (bp, Istream.count strm__) "char not terminated"
      end
  | _ ->
        ...
        [_; '\''] ->
          begin match Istream.peek strm__ with
            Some c ->
              Istream.junk strm__;
              begin match Istream.peek strm__ with
                Some '\'' -> Istream.junk strm__; Plexing.Lexbuf.add c buf
              | _ -> raise (Istream.Error "")
              end
          | _ -> raise Istream.Failure
          end
...
let any ctx buf (strm__ : _ Istream.t) =
  let bp = Istream.count strm__ in
  match Istream.peek strm__ with
    Some c -> Istream.junk strm__; ctx.line_cnt bp c; Plexing.Lexbuf.add c buf
  | _ -> raise Istream.Failure
;;

let rec string ctx bp buf (strm__ : _ Istream.t) =
  match Istream.peek strm__ with
    Some '"' -> Istream.junk strm__; buf
  | Some '\\' ->
      Istream.junk strm__;
  ...
  | _ ->
      match try Some (any ctx buf strm__) with Istream.Failure -> None with
        Some buf -> string ctx bp buf strm__
      | _ -> err ctx (bp, Istream.count strm__) "string not terminated"
;;

let comment ctx bp =
  let rec comment buf (strm__ : _ Istream.t) =
    match Istream.peek strm__ with
      Some '*' ->
        Istream.junk strm__;
        begin match Istream.peek strm__ with
    ...
    | _ ->
        ...
        | _ -> err ctx (bp, Istream.count strm__) "comment not terminated"
  in
  comment
;;

let rec quotation ctx bp buf (strm__ : _ Istream.t) =
  match Istream.peek strm__ with
    Some '>' ->
      Istream.junk strm__;
      begin match Istream.peek strm__ with
  ...
  | _ ->
      ...
      | _ -> err ctx (bp, Istream.count strm__) "quotation not terminated"
;;
...
let less ctx bp buf strm =
  if !no_quotations then
    ...
    keyword_or_error ctx (bp, Istream.count strm__) (Plexing.Lexbuf.get buf)
  else
    let (strm__ : _ Istream.t) = strm in
    match Istream.peek strm__ with
      Some '<' ->
                ...
                Some ':' ->
                  ...
                  begin match Istream.peek strm__ with
                    Some '<' ->
                      ...
                      in
                      "QUOTATION", Plexing.Lexbuf.get buf
                  | _ -> raise (Istream.Error less_expected)
                  end
              | _ -> raise Istream.Failure
        with Istream.Failure -> raise (Istream.Error "")
        end
...
let rec antiquot_rest ctx bp buf (strm__ : _ Istream.t) =
  match Istream.peek strm__ with
    Some '$' -> Istream.junk strm__; buf
  | Some '\\' ->
      Istream.junk strm__;
  ...
  | _ ->
      ...
      | _ -> err ctx (bp, Istream.count strm__) "antiquotation not terminated"
;;

let rec antiquot ctx bp buf (strm__ : _ Istream.t) =
  match Istream.peek strm__ with
    Some '$' -> Istream.junk strm__; ":" ^ Plexing.Lexbuf.get buf
  | Some ('a'..'z' | 'A'..'Z' | '0'..'9' | '!' | '_' as c) ->
      Istream.junk strm__; antiquot ctx bp (Plexing.Lexbuf.add c buf) strm__
  | Some ':' ->
      Istream.junk strm__;
...
let rec antiquot_loc ctx bp buf (strm__ : _ Istream.t) =
  match Istream.peek strm__ with
    Some '$' ->
      Istream.junk strm__;
      antiloc bp (Istream.count strm__) (":" ^ Plexing.Lexbuf.get buf)
  | Some ('a'..'z' | 'A'..'Z' | '0'..'9' | '!' | '_' as c) ->
      Istream.junk strm__;
  ...
  if ctx.dollar_for_antiquotation then
    ...
    match Istream.peek strm__ with
      Some '$' ->
        ...
        in
        begin match Istream.peek strm__ with
          Some ':' -> Istream.junk strm__; "ANTIQUOT", "?" ^ s ^ ":"
        | _ -> "ANTIQUOT", "?" ^ s
        end
    | _ ->
        let (strm__ : _ Istream.t) = strm in
    ...
    match Istream.peek strm__ with
      Some '$' ->
        ...
        in
        begin match Istream.peek strm__ with
          Some ':' -> Istream.junk strm__; "ANTIQUOT_LOC", "?" ^ s ^ ":"
        | _ -> "ANTIQUOT_LOC", "?" ^ s
        end
    | _ ->
        let (strm__ : _ Istream.t) = strm in
...
let tildeident buf (strm__ : _ Istream.t) =
  match Istream.peek strm__ with
    Some ':' -> Istream.junk strm__; "TILDEIDENTCOLON", Plexing.Lexbuf.get buf
  | _ -> "TILDEIDENT", Plexing.Lexbuf.get buf
;;

let questionident buf (strm__ : _ Istream.t) =
  match Istream.peek strm__ with
    Some ':' ->
      Istream.junk strm__; "QUESTIONIDENTCOLON", Plexing.Lexbuf.get buf
...
let rec any_to_nl buf (strm__ : _ Istream.t) =
  match Istream.peek strm__ with
    Some ('\r' | '\n' as c) -> Istream.junk strm__; Plexing.Lexbuf.add c buf
  | Some c -> Istream.junk strm__; any_to_nl (Plexing.Lexbuf.add c buf) strm__
  | _ -> buf
;;

let next_token_after_spaces ctx bp buf (strm__ : _ Istream.t) =
  match Istream.peek strm__ with
    Some ('A'..'Z' as c) ->
      Istream.junk strm__;
  ...
  | _ ->
      match
        try Some (greek_letter buf strm__) with Istream.Failure -> None
      with
        Some buf ->
      ...
      | _ ->
          ...
          | _ ->
              match Istream.peek strm__ with
                Some ('1'..'9' as c) ->
                  Istream.junk strm__;
                  number (Plexing.Lexbuf.add c buf) strm__
              | Some '0' ->
                  Istream.junk strm__;
                  begin match Istream.peek strm__ with
                    Some ('o' | 'O' as c) ->
              ...
              | Some '\'' ->
                  Istream.junk strm__;
                  begin match Istream.npeek 3 strm__ with
                    ['\\'; 'a'..'z'; 'a'..'z'] ->
                      keyword_or_error ctx (bp, Istream.count strm__) "'"
                  | _ ->
                      match
              ...
              | Some '$' -> Istream.junk strm__; dollar ctx bp buf strm__
              | Some
                  ('!' | '=' | '@' | '^' | '&' | '+' | '-' | '*' | '/' |
                   '%' as c) ->
                  Istream.junk strm__;
              ...
              | Some '<' -> Istream.junk strm__; less ctx bp buf strm__
              | Some ':' ->
                  Istream.junk strm__;
                  begin match Istream.peek strm__ with
                    Some ']' ->
              ...
              | Some '[' ->
                  Istream.junk strm__;
                  begin match Istream.npeek 2 strm__ with
                    ['<'; '<'] | ['<'; ':'] ->
                      keyword_or_error ctx (bp, Istream.count strm__)
                        (Plexing.Lexbuf.get (Plexing.Lexbuf.add '[' buf))
                  | _ ->
              ...
              | Some '.' ->
                  ...
                  begin match Istream.peek strm__ with
                    Some '.' ->
                      ...
                      keyword_or_error ctx (bp, Istream.count strm__) ".."
                  | _ ->
                      match Istream.npeek 1 strm__ with
                        ['\n'] ->
                      ...
                      | _ ->
                          ...
                          in
                          keyword_or_error ctx (bp, Istream.count strm__) id
                  end
              | Some ';' ->
                  Istream.junk strm__;
                  begin match Istream.peek strm__ with
                    Some ';' ->
                      Istream.junk strm__;
                      keyword_or_error ctx (bp, Istream.count strm__) ";;"
                  | _ -> keyword_or_error ctx (bp, Istream.count strm__) ";"
                  end
              | _ ->
...
let rec next_token ctx buf (strm__ : _ Istream.t) =
  let bp = Istream.count strm__ in
  match Istream.peek strm__ with
    Some ('\n' | '\r' as c) ->
      Istream.junk strm__;
  ...
  | Some '#' when bp = !(!(Plexing.bol_pos)) ->
      ...
      if linedir 1 s then
        ...
        !(Plexing.bol_pos) := Istream.count s;
        ctx.set_line_nb ();
        ctx.after_space <- true;
...
let rec check_keyword_stream (strm__ : _ Istream.t) =
  let _ = check Plexing.Lexbuf.empty strm__ in
  let _ =
    try Istream.empty strm__ with Istream.Failure -> raise (Istream.Error "")
  in
  true
and check buf (strm__ : _ Istream.t) =
  match
    try
      Some
        (match Istream.peek strm__ with
  ...
  | _ ->
      match Istream.peek strm__ with
        Some
          ('!' | '?' | '~' | '=' | '@' | '^' | '&' | '+' | '-' | '*' | '/' |
           '%' | '.' as c) ->
          Istream.junk strm__; check_ident2 (Plexing.Lexbuf.add c buf) strm__
      | Some '$' ->
          Istream.junk strm__;
          check_ident2 (Plexing.Lexbuf.add '$' buf) strm__
      | Some '<' ->
          Istream.junk strm__;
          begin match Istream.npeek 1 strm__ with
            [':'] | ['<'] -> Plexing.Lexbuf.add '<' buf
      ...
      | _ ->
          match
            try Some (misc_punct buf strm__) with Istream.Failure -> None
          with
            Some buf -> check_ident2 buf strm__
          | _ ->
              match Istream.peek strm__ with
                Some c -> Istream.junk strm__; Plexing.Lexbuf.add c buf
              | _ -> raise Istream.Failure
and check_ident buf (strm__ : _ Istream.t) =
  match
    try
      Some
        (match Istream.peek strm__ with
  ...
  with
    Some buf -> check_ident buf strm__
  | _ -> buf
and check_ident2 buf (strm__ : _ Istream.t) =
  match
    try
      Some
        (match Istream.peek strm__ with
