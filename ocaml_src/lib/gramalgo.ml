...
let possible_name =
  ...
  | Stoken p ->
      let s = name_of_token p in if good_ident_name s then Some s else None
  | _ -> None
;;
...
let name_of_meta =
  function
    Slist0 s ->
      begin match possible_name s with
        Some s -> "list0-" ^ s
      | _ -> "list0"
      end
  | Slist0sep (s, sep) ->
      begin match possible_name s, possible_name sep with
...
let make_anon_rules anon_rules suff cnt s =
  let rec loop =
    function
      (n, s1) :: rest -> if Gramext.eq_symbol s s1 then n else loop rest
    | [] ->
        incr cnt;
        let n = "x" ^ string_of_int !cnt ^ "-" ^ suff in
        ...
        let gs =
          ...
          | Slist0 _ | Slist0sep (_, _) | Slist1 _ | Slist1sep (_, _) |
            Sopt _ | Sflag _ ->
              ...
              GS_nterm n
          | Stoken p -> let n = name_of_token p in GS_term n
          | Sself -> self_middle ()
          | Stree _ ->
              incr cnt;
