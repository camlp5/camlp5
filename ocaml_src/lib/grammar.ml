...
let rec fparser_of_tree entry nlevn alevn =
  ...
  | Node {node = Sself; son = LocAct (act, _); brother = DeadEnd} ->
      (fun (strm__ : _ Fstream.t) ->
         match entry.fstart alevn strm__ with
...
and fparser_of_symbol entry nlevn =
              ...
              begin match
                (fun (strm__ : _ Fstream.t) ->
                   match ps al strm__ with
                     Some _ as x -> x
                   | None ->
                       match fparse_top_symb entry symb strm__ with
  ...
  | Snext -> (fun (strm__ : _ Fstream.t) -> entry.fstart nlevn strm__)
...
and fparser_of_token entry tok =
  ...
  fun strm ->
    match Fstream.next strm with
      Some (tok, strm) ->
        begin try let r = f tok in Some (Obj.repr r, strm) with
          Stream.Failure -> None
        end
    | None -> None
...
let fcount strm = Some (Fstream.count strm, strm);;
...
module Entry =
  struct
    ...
    let parse_parsable (entry : 'a e) p : 'a =
      ...
      | ImperativeStreams -> Obj.magic (parse_parsable entry p : Obj.t)
      | FunctionalStreams -> Obj.magic (fparse_parsable entry p : Obj.t)
    ;;
...
let create_local_entry g n =
  {egram = g; ename = n; elocal = true; estart = empty_entry n;
   econtinue = (fun _ _ _ (strm__ : _ Stream.t) -> raise Stream.Failure);
   fstart = (fun _ (strm__ : _ Fstream.t) -> None);
   fcontinue = (fun _ _ _ (strm__ : _ Fstream.t) -> None); edesc = Dlevels []}
;;
...
let clear_entry e =
  ...
  e.fstart <- (fun _ (strm__ : _ Fstream.t) -> None);
  e.fcontinue <- (fun _ _ _ (strm__ : _ Fstream.t) -> None);
  match e.edesc with
    ...
    module Entry =
      struct
        ...
        let parse (e : 'a e) p : 'a =
          ...
          | ImperativeStreams -> Obj.magic (parse_parsable e p : Obj.t)
          | FunctionalStreams -> Obj.magic (fparse_parsable e p : Obj.t)
        ;;
