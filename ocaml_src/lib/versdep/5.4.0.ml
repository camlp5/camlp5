...
let ocaml_longident_ldot (loc : Location.t) li s =
  Ldot (mkloc loc li, mkloc loc s)
;;

let ocaml_longident_lapply (loc : Location.t) li1 li2 =
  Lapply (mkloc loc li1, mkloc loc li2)
;;
let ocaml_longident_lident (loc : Location.t) s = Lident s;;

let ocaml_longident_option_of_string_list (loc : Location.t) sl =
  match sl with
    [] -> None
  | s :: sl ->
      let rec loop acc =
...
let ocaml_ptyp_open loc li t = Ptyp_open (mkloc loc li, t);;
      ...
      Pexp_extension
        (eid,
         PStr
           [{pstr_desc =
              Pstr_eval
                ({pexp_desc = Pexp_letexception (exdef, body); pexp_loc = loc;
                  pexp_loc_stack = []; pexp_attributes = attrs},
                 []);
             pstr_loc = loc}])
       ...
       Pstr_extension
         ((eid,
           PStr
             [{pstr_desc =
                 Pstr_module
                   {pmb_name = name; pmb_expr = mexpr; pmb_attributes = attrs};
               pstr_loc = loc}]),
          _)} ->
      Pexp_extension
        (eid,
       ...
       Pstr_extension
         ((eid,
           PStr
             [{pstr_desc =
                 Pstr_open
                   {popen_expr = mexpr; popen_override = ovf;
           ...
           [{pstr_desc =
              Pstr_eval
                ({pexp_desc =
                   Pexp_open
                     ({popen_expr = mexpr; popen_override = ovf;
                       popen_loc = loc; popen_attributes = []},
                      body);
                  pexp_loc = loc; pexp_loc_stack = [];
                  pexp_attributes = attrs},
                 []);
...
let ocaml_ec_tuple ?(alg_attributes = []) loc s tyvars (x, rto) =
  ...
  {pext_name = mkloc loc s;
   pext_kind = Pext_decl (tyvars, Pcstr_tuple x, rto); pext_loc = loc;
      ...
      List.map
        (fun (c, tl, loc, attrs) ->
           let (tyvars, tl, rto) =
             ...
             | _ -> assert false
           in
           {pcd_name = mkloc loc c; pcd_vars = List.map (mkloc loc) tyvars;
            pcd_args = tl; pcd_res = rto; pcd_loc = loc;
...
let ocaml_ptyp_functor loc idopt mid pt ct : core_type_desc =
  failwith "Ptyp_functor: only available with ocaml >= 5.05"
;;
...
let ocaml_ppat_tuple l cflag = Ppat_tuple (l, cflag);;
...
let ocaml_ppat_unpack loc s = Ppat_unpack (mkloc loc s);;
