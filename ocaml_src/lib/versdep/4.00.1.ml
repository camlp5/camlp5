...
let split_on_char sep s =
  ...
  for i = length s - 1 downto 0 do
    if unsafe_get s i = sep then
      begin r := sub s (i + 1) (!j - i - 1) :: !r; j := i end
  done;
  sub s 0 !j :: !r
;;
    ...
    let ctl =
      List.map
        (fun (c, tl, loc, attrs) ->
           let (tl, rto) =
             match tl with
               Left (tyvar, x), y -> assert ([] = tyvars); x, y
  ...
  Some
    (fun ovf me e ->
       let li =
         ...
         | _ -> assert false
       in
       assert (ovf = Fresh); Pexp_open (li, e))
;;
...
let ocaml_psig_exception ?(alg_attributes = []) ?(item_attributes = []) loc s
  ...
  in
  assert (None = rto); assert ([] = tyvars); Psig_exception (mkloc loc s, ed)
;;
...
let ocaml_psig_open ?(item_attributes = []) loc li =
  assert (item_attributes = []); Psig_open (mkloc loc li)
;;
...
let ocaml_pstr_exception ?(alg_attributes = []) ?(item_attributes = []) loc s
  ...
  in
  assert ([] = tyvars); assert (None = rto); Pstr_exception (mkloc loc s, ed)
;;
