...
let gram =
  Grammar.gcreate
    {Token.tok_func = (fun _ -> failwith "no loaded parsing module");
     ...
     Token.tok_match = (fun _ -> raise (Match_failure ("pcaml.ml", 40, 23)));
...
;;
let pr_expr =
  {pr_fun = (fun _ -> raise (Match_failure ("pcaml.ml", 405, 26)));
   pr_levels = []}
;;
let pr_patt =
...
let pr_class_sig_item =
  {pr_fun = (fun _ -> raise (Match_failure ("pcaml.ml", 408, 36)));
   pr_levels = []}
;;
...
;;
let pr_class_type =
  {pr_fun = (fun _ -> raise (Match_failure ("pcaml.ml", 410, 32)));
   pr_levels = []}
;;
let pr_class_expr =
