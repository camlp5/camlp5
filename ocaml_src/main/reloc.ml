...
let class_infos_map floc sh ~attributes f x =
  {ciLoc = floc x.ciLoc; ciVir = x.ciVir;
   ciPrm = (let (x1, x2) = x.ciPrm in floc x1, x2); ciNam = x.ciNam;
   ciExp = f x.ciExp; ciAttributes = attributes floc sh x.ciAttributes}
;;
    ...
    | TyObj (loc, x1, x2) ->
        ...
        TyObj
          (loc,
           vala_map
             (List.map
                (fun (x1, x2, x3) -> x1, self x2, attributes floc sh x3))
             x1,
           x2)
        ...
        TyRec
          (loc,
           vala_map
             (List.map
                (fun (loc, x1, x2, x3, x4) ->
                   floc loc, x1, x2, self x3, attributes floc sh x4))
             x1)
    | TySum (loc, x1) ->
        let loc = floc loc in
...
and poly_variant floc sh =
  function
    PvTag (loc, x1, x2, x3, x4) ->
      let loc = floc loc in
      PvTag
        (loc, x1, x2, vala_map (List.map (ctyp floc sh)) x3,
...
and expr floc sh =
  let rec self =
    ...
    | ExFor (loc, x1, x2, x3, x4, x5) ->
        let loc = floc loc in
        ExFor
          (loc, patt floc sh x1, self x2, self x3, x4,
...
and sig_item floc sh =
  let rec self =
      ...
      SgCls (loc, x1) ->
        ...
        SgCls
          (loc,
           vala_map
             (List.map
                (class_infos_map floc sh ~attributes:attributes
                   (class_type floc sh)))
             x1)
    | SgClt (loc, x1) ->
        let loc = floc loc in
        ...
        SgMod
          (loc, x1,
           vala_map
             (List.map
                (fun (x1, x2, x3) ->
                   x1, module_type floc sh x2, attributes floc sh x3))
             x2)
    | SgMty (loc, x1, x2, x3) ->
        let loc = floc loc in
        SgMty (loc, x1, module_type floc sh x2, attributes floc sh x3)
    | SgMtyAbs (loc, x1, x2) ->
        let loc = floc loc in SgMtyAbs (loc, x1, attributes floc sh x2)
    | SgMtyAlias (loc, x1, x2, x3) ->
        let loc = floc loc in
        SgMtyAlias
          (loc, x1, vala_map (longid floc sh) x2, attributes floc sh x3)
    ...
    | SgTypExten (loc, x1) ->
        let loc = floc loc in SgTypExten (loc, type_extension floc sh x1)
    | SgUse (loc, x1, x2) ->
        let loc = floc loc in
        SgUse
          (loc, x1,
...
and with_constr floc sh =
  ...
  | WcTyp (loc, x1, x2, x3, x4) ->
      let loc = floc loc in
      WcTyp
        (loc, vala_map (longid_lident floc sh) x1, x2, x3, ctyp floc sh x4)
  | WcTys (loc, x1, x2, x3) ->
      let loc = floc loc in
...
and module_expr floc sh =
  let rec self =
    ...
    | MeUnp (loc, x1, x2, x3) ->
        let loc = floc loc in
        MeUnp
          (loc, expr floc sh x1, option_map (module_type floc sh) x2,
...
and str_item floc sh =
  let rec self =
    ...
    | StExc (loc, x1, x2) ->
        let loc = floc loc in
        StExc
          (loc, vala_map (extension_constructor floc sh) x1,
           attributes floc sh x2)
    | StExp (loc, x1, x2) ->
        let loc = floc loc in
        ...
        StMod
          (loc, x1,
           vala_map
             (List.map
                (fun (x1, x2, x3) ->
                   x1, module_expr floc sh x2, attributes floc sh x3))
             x2)
    | StMty (loc, x1, x2, x3) ->
        let loc = floc loc in
        StMty (loc, x1, module_type floc sh x2, attributes floc sh x3)
    | StMtyAbs (loc, x1, x2) ->
        let loc = floc loc in StMtyAbs (loc, x1, attributes floc sh x2)
    | StOpn (loc, x1, x2, x3) ->
        let loc = floc loc in
    ...
    | StTypExten (loc, x1) ->
        let loc = floc loc in StTypExten (loc, type_extension floc sh x1)
    | StUse (loc, x1, x2) ->
        let loc = floc loc in
        StUse
          (loc, x1,
        ...
        StVal
          (loc, x1,
           vala_map
             (List.map
                (fun (x1, x2, x3) ->
                   patt floc sh x1, expr floc sh x2, attributes floc sh x3))
             x2)
    | StXtr (loc, x1, x2) ->
...
and extension_constructor floc sh =
  function
    EcTuple (loc, x1) -> EcTuple (floc loc, generic_constructor floc sh x1)
  | EcRebind (loc, x1, x2, x3) ->
      EcRebind (floc loc, x1, x2, attributes floc sh x3)
and class_type floc sh =
  let rec self =
    ...
    | CtLop (loc, x1, x2, x3) ->
        let loc = floc loc in CtLop (loc, x1, longid floc sh x2, self x3)
    | CtCon (loc, x1, x2) ->
        let loc = floc loc in
    ...
    | CeLet (loc, x1, x2, x3) ->
        ...
        CeLet
          (loc, x1,
           ...
           self x3)
    | CeLop (loc, x1, x2, x3) ->
        let loc = floc loc in CeLop (loc, x1, longid floc sh x2, self x3)
    | CeStr (loc, x1, x2) ->
        let loc = floc loc in
        CeStr
          (loc, vala_map (option_map (patt floc sh)) x1,
