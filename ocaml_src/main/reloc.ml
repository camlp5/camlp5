...
let rec ctyp floc sh =
  let rec self =
    ...
    | TyCls (loc, x1, x2) ->
        let loc = floc loc in TyCls (loc, option_map (longid floc sh) x1, x2)
    ...
    | TyObj (loc, x1, x2) ->
        let loc = floc loc in
        TyObj
          (loc, vala_map (List.map (fun (x1, x2, x3) -> x1, self x2, x3)) x1,
           x2)
    ...
    | TyRec (loc, x1) ->
        ...
        TyRec
          (loc,
           vala_map
             (List.map
                (fun (loc, x1, x2, x3, x4) -> floc loc, x1, x2, self x3, x4))
             x1)
    | TySum (loc, x1) ->
        let loc = floc loc in
        ...
        ExMat
          (loc, self x1,
           vala_map
             ...
             x2)
    | ExNew (loc, x1, x2) ->
        let loc = floc loc in ExNew (loc, option_map (longid floc sh) x1, x2)
    | ExObj (loc, x1, x2) ->
        let loc = floc loc in
        ExObj
...
and sig_item floc sh =
  let rec self =
    ...
    | SgExc (loc, x1, x2) ->
        let loc = floc loc in SgExc (loc, generic_constructor floc sh x1, x2)
    | SgExt (loc, x1, x2, x3, x4) ->
        let loc = floc loc in SgExt (loc, x1, ctyp floc sh x2, x3, x4)
    | SgInc (loc, x1, x2) ->
        let loc = floc loc in SgInc (loc, module_type floc sh x1, x2)
    | SgMod (loc, x1, x2) ->
        let loc = floc loc in
        SgMod
          (loc, x1,
...
and str_item floc sh =
  let rec self =
    ...
    | StMtyAbs (loc, x1, x2) -> let loc = floc loc in StMtyAbs (loc, x1, x2)
    | StOpn (loc, x1, x2, x3) ->
        let loc = floc loc in StOpn (loc, x1, module_expr floc sh x2, x3)
    | StTyp (loc, x1, x2) ->
        let loc = floc loc in
...
and class_expr floc sh =
  let rec self =
    ...
    | CeCon (loc, x1, x2, x3) ->
        let loc = floc loc in
        CeCon
          (loc, option_map (longid floc sh) x1, x2,
           vala_map (List.map (ctyp floc sh)) x3)
    | CeFun (loc, x1, x2) ->
