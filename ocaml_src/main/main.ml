...
let print_verbose_version () =
  printf "Camlp5 version %s (ocaml %s)\n" Pcaml.version Pcaml.ocaml_version;
  flush stdout;
  exit 0
;;

let print_succinct_version () =
  printf "%s\n" Pcaml.version; flush stdout; exit 0
;;
...
let initial_spec_list =
  ["-intf", Arg.String (fun x -> file_kind := Intf; Pcaml.input_file := x),
   ...
   "-version", Arg.Unit print_succinct_version,
   "Print succinct Camlp5 version and exit.";
   "-v", Arg.Unit print_verbose_version,
