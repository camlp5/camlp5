...
module PA (Base : Mlsyntax.PARSEBASESIG)
  ...
  (Q_ast_base :
   Parse_q_ast_base.PARSE_Q_AST_BASE_SIG with
     module Base = Base
     and module QH = QH) =
  struct
    module Base = Base;;
    ...
    module Meta_make (C : MetaSig) =
      struct
        module C = C;;
        open MLast;;
        let variance v = C.vala C.string v;;
        let type_var (s, tv) =
        ...
        let record_label lab =
          ...
          MLast.PaPfx
            (loc, MLast.LiUid (loc, "MLast"), MLast.PaLid (loc, lab))
        ;;
        ...
        let rec ctyp =
          ...
          | TyAcc (_, m1, t2) ->
              C.node "TyAcc" [longid m1; C.vala C.string t2]
          ...
          | TyExt (loc, s) -> C.node "TyExt" [C.vala C.string s]
          | TyExten (loc, exten) ->
              let exten = conv_extension exten in C.node "TyExten" [exten]
          | TyOpen (_, li, t) -> C.node "TyOpen" [longid li; ctyp t]
          | TyFun (_, arg, s, mty, cty) ->
              C.node "TyFun"
                [C.vala (C.option (C.vala C.string)) arg; C.vala C.string s;
                 module_type mty; ctyp cty]
        ...
        and patt =
          ...
          | PaLong (_, li, loc_ids) ->
              C.node "PaLong"
                [longid li;
                 C.vala
                   (C.list (fun (_, s) -> C.tuple [C.loc_v (); C.string s]))
                   loc_ids]
          ...
          | PaInt (_, s1, s2) ->
              C.node "PaInt" [C.vala C.string s1; C.string s2]
          | PaLab (_, p, op) ->
              C.node "PaLab" [patt p; C.vala (C.option patt) op]
          | PaLaz (_, p) -> C.node "PaLaz" [patt p]
          | PaLid (_, s) -> C.node "PaLid" [C.vala C.string s]
          | PaNty (_, s) -> C.node "PaNty" [C.vala C.string s]
          | PaOlb (_, p, oe) ->
              C.node "PaOlb" [patt p; C.vala (C.option expr) oe]
          | PaOrp (_, p1, p2) -> C.node "PaOrp" [patt p1; patt p2]
          | PaRec (_, lpp) ->
              C.node "PaRec"
                [C.vala (C.list (fun (p1, p2) -> C.tuple [patt p1; patt p2]))
                   lpp]
        ...
        and expr =
          ...
          | ExFun (_, lpoee) ->
              C.node "ExFun"
                [C.vala
                   ...
                   lpoee]
          | ExIfe (_, e1, e2, e3) ->
              C.node "ExIfe" [expr e1; expr e2; expr e3]
          | ExInt (_, s1, s2) ->
              C.node "ExInt" [C.vala C.string s1; C.string s2]
          | ExLab (_, p, oe) ->
              C.node "ExLab" [patt p; C.vala (C.option expr) oe]
          ...
          | ExOlb (_, p, oe) ->
              C.node "ExOlb" [patt p; C.vala (C.option expr) oe]
          | ExOvr (_, lse) ->
              C.node "ExOvr"
          ...
          | ExStr (_, s) ->
              C.node "ExStr"
                [C.vala
                   (fun (n, s) -> C.tuple [C.loc_v (); C.vala C.string s]) s]
          | ExTry (_, e, lpoee) ->
              C.node "ExTry"
                [expr e;
                 C.vala
          ...
          | ExWhi (_, e, le) ->
              C.node "ExWhi" [expr e; C.vala (C.list expr) le]
          | ExXtr (loc, s, _) ->
                 ...
                 C.vala
                   (C.list
                      (fun (sopt, mt, attrs) ->
                         let attrs = conv_attributes attrs in
                         C.tuple
                           [C.vala c_vala_opt sopt; module_type mt; attrs]))
                   lsmt]
          | SgMty (_, s, mt, attrs) ->
              let attrs = conv_attributes attrs in
        ...
        and longid =
          function
            LiAcc (_, me1, s) ->
              C.node "LiAcc" [longid me1; C.vala C.string s]
        ...
        and module_expr =
          ...
          | MeTyc (_, me, mt) ->
              C.node "MeTyc" [module_expr me; module_type mt]
                 ...
                 C.vala
                   (C.list
                      (fun (sopt, me, attrs) ->
                         let attrs = conv_attributes attrs in
                         C.tuple
                           [C.vala c_vala_opt sopt; module_expr me; attrs]))
                   lsme]
          | StMty (_, s, mt, attrs) ->
              let attrs = conv_attributes attrs in
        ...
        and class_sig_item =
          ...
          | CgMth (_, b, s, t, attrs) ->
              let attrs = conv_attributes attrs in
              C.node "CgMth"
                [C.vala C.bool b; C.vala C.string s; ctyp t; attrs]
          | CgVal (_, mf, vf, s, t, attrs) ->
              let attrs = conv_attributes attrs in
              C.node "CgVal"
                [C.vala C.bool mf; C.vala C.bool vf; C.vala C.string s;
                 ctyp t; attrs]
          | CgVir (_, b, s, t, attrs) ->
              let attrs = conv_attributes attrs in
              C.node "CgVir"
        ...
        and class_str_item =
          ...
          | CrInh (loc, b1, ce, os, attrs) ->
              let attrs = conv_attributes attrs in
              C.node "CrInh"
                [C.vala C.bool b1; class_expr ce;
                 C.vala (C.option C.string) os; attrs]
          | CrIni (_, e, attrs) ->
              let attrs = conv_attributes attrs in
    ...
    let attribute_body_eoi =
      Grammar.Entry.create Base.Parsers.gram "attribute_body_eoi"
    ;;
    let class_expr_eoi = Grammar.Entry.create Base.Parsers.gram "class_expr";;
    let class_sig_item_eoi =
      Grammar.Entry.create Base.Parsers.gram "class_sig_item"
    ;;
    let class_str_item_eoi =
      Grammar.Entry.create Base.Parsers.gram "class_str_item"
    ;;
    let class_type_eoi = Grammar.Entry.create Base.Parsers.gram "class_type";;
    let ctyp_eoi = Grammar.Entry.create Base.Parsers.gram "type";;
    let expr_eoi = Grammar.Entry.create Base.Parsers.gram "expr";;
    let extended_longident_eoi =
      Grammar.Entry.create Base.Parsers.gram "extended_longident_eoi"
    ;;
    let extension_constructor_eoi =
      Grammar.Entry.create Base.Parsers.gram "extension_constructor_eoi"
    ;;
    let constructor_eoi =
      Grammar.Entry.create Base.Parsers.gram "constructor_eoi"
    ;;
    let longident_eoi =
      Grammar.Entry.create Base.Parsers.gram "longident_eoi"
    ;;
    let module_expr_eoi =
      Grammar.Entry.create Base.Parsers.gram "module_expr"
    ;;
    let module_type_eoi =
      Grammar.Entry.create Base.Parsers.gram "module_type"
    ;;
    let patt_eoi = Grammar.Entry.create Base.Parsers.gram "patt";;
    let poly_variant_eoi =
      Grammar.Entry.create Base.Parsers.gram "poly_variant"
    ;;
    let sig_item_eoi = Grammar.Entry.create Base.Parsers.gram "sig_item";;
    let str_item_eoi = Grammar.Entry.create Base.Parsers.gram "str_item";;
    let type_decl_eoi =
      Grammar.Entry.create Base.Parsers.gram "type_declaration"
    ;;
    let type_extension_eoi =
      Grammar.Entry.create Base.Parsers.gram "type_extension_eoi"
    ;;
    let with_constr_eoi =
      Grammar.Entry.create Base.Parsers.gram "with_constr"
    ;;
    Grammar.safe_extend
      [Grammar.extension
          ...
          [Grammar.production
             (Grammar.r_next
                (Grammar.r_next Grammar.r_stop
                   (Grammar.s_nterm
                      (Base.Parsers.attribute_body :
                       'Base__Parsers__attribute_body Grammar.Entry.e)))
                (Grammar.s_token ("EOI", "")),
              "194fe98d",
              (fun _ (x : 'Base__Parsers__attribute_body) (loc : Ploc.t) ->
         ...
         [None, None,
          [Grammar.production
             (Grammar.r_next
                (Grammar.r_next Grammar.r_stop
                   (Grammar.s_nterm
                      (Base.Parsers.class_expr :
                       'Base__Parsers__class_expr Grammar.Entry.e)))
                (Grammar.s_token ("EOI", "")),
              "194fe98d",
              (fun _ (x : 'Base__Parsers__class_expr) (loc : Ploc.t) ->
                 (x : 'class_expr_eoi)))]];
       Grammar.extension
         ...
         [None, None,
          [Grammar.production
             (Grammar.r_next
                (Grammar.r_next Grammar.r_stop
                   (Grammar.s_nterm
                      (Base.Parsers.class_sig_item :
                       'Base__Parsers__class_sig_item Grammar.Entry.e)))
                (Grammar.s_token ("EOI", "")),
              "194fe98d",
              (fun _ (x : 'Base__Parsers__class_sig_item) (loc : Ploc.t) ->
         ...
         [None, None,
          [Grammar.production
             (Grammar.r_next
                (Grammar.r_next Grammar.r_stop
                   (Grammar.s_nterm
                      (Base.Parsers.class_str_item :
                       'Base__Parsers__class_str_item Grammar.Entry.e)))
                (Grammar.s_token ("EOI", "")),
              "194fe98d",
              (fun _ (x : 'Base__Parsers__class_str_item) (loc : Ploc.t) ->
         ...
         [None, None,
          [Grammar.production
             (Grammar.r_next
                (Grammar.r_next Grammar.r_stop
                   (Grammar.s_nterm
                      (Base.Parsers.class_type :
                       'Base__Parsers__class_type Grammar.Entry.e)))
                (Grammar.s_token ("EOI", "")),
              "194fe98d",
              (fun _ (x : 'Base__Parsers__class_type) (loc : Ploc.t) ->
                 (x : 'class_type_eoi)))]];
          ...
          [Grammar.production
             (Grammar.r_next
                (Grammar.r_next Grammar.r_stop
                   (Grammar.s_nterm
                      (Base.Parsers.ctyp :
                       'Base__Parsers__ctyp Grammar.Entry.e)))
                (Grammar.s_token ("EOI", "")),
              "194fe98d",
              (fun _ (x : 'Base__Parsers__ctyp) (loc : Ploc.t) ->
                 (x : 'ctyp_eoi)))]];
         ...
         [None, None,
          [Grammar.production
             (Grammar.r_next
                ...
                (Grammar.s_token ("EOI", "")),
              "194fe98d",
              (fun _ (x : 'Base__Parsers__expr) (loc : Ploc.t) ->
                 (x : 'expr_eoi)))]];
       Grammar.extension
         ...
         [None, None,
          [Grammar.production
             (Grammar.r_next
                (Grammar.r_next Grammar.r_stop
                   (Grammar.s_nterm
                      (Base.Parsers.extended_longident :
                       'Base__Parsers__extended_longident Grammar.Entry.e)))
                (Grammar.s_token ("EOI", "")),
              "194fe98d",
              (fun _ (x : 'Base__Parsers__extended_longident)
                   (loc : Ploc.t) ->
         ...
         [None, None,
          [Grammar.production
             (Grammar.r_next
                (Grammar.r_next Grammar.r_stop
                   (Grammar.s_nterm
                      (Base.Parsers.extension_constructor :
                       'Base__Parsers__extension_constructor
                         Grammar.Entry.e)))
                (Grammar.s_token ("EOI", "")),
              "194fe98d",
              (fun _ (x : 'Base__Parsers__extension_constructor)
                   (loc : Ploc.t) ->
         ...
         [None, None,
          [Grammar.production
             (Grammar.r_next
                (Grammar.r_next Grammar.r_stop
                   (Grammar.s_nterm
                      (Base.Parsers.constructor_declaration :
                       'Base__Parsers__constructor_declaration
                         Grammar.Entry.e)))
                (Grammar.s_token ("EOI", "")),
              "194fe98d",
              (fun _ (x : 'Base__Parsers__constructor_declaration)
                   (loc : Ploc.t) ->
                 (x : 'constructor_eoi)))]];
         ...
         [None, None,
          [Grammar.production
             (Grammar.r_next
                (Grammar.r_next Grammar.r_stop
                   (Grammar.s_nterm
                      (Base.Parsers.longident :
                       'Base__Parsers__longident Grammar.Entry.e)))
                (Grammar.s_token ("EOI", "")),
              "194fe98d",
              (fun _ (x : 'Base__Parsers__longident) (loc : Ploc.t) ->
                 (x : 'longident_eoi)))]];
         ...
         [None, None,
          [Grammar.production
             (Grammar.r_next
                (Grammar.r_next Grammar.r_stop
                   (Grammar.s_nterm
                      (Base.Parsers.module_expr :
                       'Base__Parsers__module_expr Grammar.Entry.e)))
                (Grammar.s_token ("EOI", "")),
              "194fe98d",
              (fun _ (x : 'Base__Parsers__module_expr) (loc : Ploc.t) ->
                 (x : 'module_expr_eoi)))]];
         ...
         [None, None,
          [Grammar.production
             (Grammar.r_next
                (Grammar.r_next Grammar.r_stop
                   (Grammar.s_nterm
                      (Base.Parsers.module_type :
                       'Base__Parsers__module_type Grammar.Entry.e)))
                (Grammar.s_token ("EOI", "")),
              "194fe98d",
              (fun _ (x : 'Base__Parsers__module_type) (loc : Ploc.t) ->
                 (x : 'module_type_eoi)))]];
         ...
         [None, None,
          [Grammar.production
             (Grammar.r_next
                ...
                (Grammar.s_token ("EOI", "")),
              "194fe98d",
              (fun _ (x : 'Base__Parsers__patt) (loc : Ploc.t) ->
                 (x : 'patt_eoi)))]];
       Grammar.extension
         ...
         [None, None,
          [Grammar.production
             (Grammar.r_next
                (Grammar.r_next Grammar.r_stop
                   (Grammar.s_nterm
                      (Base.Parsers.poly_variant :
                       'Base__Parsers__poly_variant Grammar.Entry.e)))
                (Grammar.s_token ("EOI", "")),
              "194fe98d",
              (fun _ (x : 'Base__Parsers__poly_variant) (loc : Ploc.t) ->
                 (x : 'poly_variant_eoi)))]];
         ...
         [None, None,
          [Grammar.production
             (Grammar.r_next
                (Grammar.r_next Grammar.r_stop
                   (Grammar.s_nterm
                      (Base.Parsers.sig_item :
                       'Base__Parsers__sig_item Grammar.Entry.e)))
                (Grammar.s_token ("EOI", "")),
              "194fe98d",
              (fun _ (x : 'Base__Parsers__sig_item) (loc : Ploc.t) ->
                 (x : 'sig_item_eoi)))]];
         ...
         [None, None,
          [Grammar.production
             (Grammar.r_next
                (Grammar.r_next Grammar.r_stop
                   (Grammar.s_nterm
                      (Base.Parsers.str_item :
                       'Base__Parsers__str_item Grammar.Entry.e)))
                (Grammar.s_token ("EOI", "")),
              "194fe98d",
              (fun _ (x : 'Base__Parsers__str_item) (loc : Ploc.t) ->
                 (x : 'str_item_eoi)))]];
         ...
         [None, None,
          [Grammar.production
             (Grammar.r_next
                (Grammar.r_next Grammar.r_stop
                   (Grammar.s_nterm
                      (Base.Parsers.type_decl :
                       'Base__Parsers__type_decl Grammar.Entry.e)))
                (Grammar.s_token ("EOI", "")),
              "194fe98d",
              (fun _ (x : 'Base__Parsers__type_decl) (loc : Ploc.t) ->
                 (x : 'type_decl_eoi)))]];
       Grammar.extension
         ...
         [None, None,
          [Grammar.production
             (Grammar.r_next
                (Grammar.r_next Grammar.r_stop
                   (Grammar.s_nterm
                      (Base.Parsers.type_extension :
                       'Base__Parsers__type_extension Grammar.Entry.e)))
                (Grammar.s_token ("EOI", "")),
              "194fe98d",
              (fun _ (x : 'Base__Parsers__type_extension) (loc : Ploc.t) ->
         ...
         [None, None,
          [Grammar.production
             (Grammar.r_next
                (Grammar.r_next Grammar.r_stop
                   (Grammar.s_nterm
                      (Base.Parsers.with_constr :
                       'Base__Parsers__with_constr Grammar.Entry.e)))
                (Grammar.s_token ("EOI", "")),
              "194fe98d",
              (fun _ (x : 'Base__Parsers__with_constr) (loc : Ploc.t) ->
                 (x : 'with_constr_eoi)))]]];;
    ...
    let apply_entry e me mp =
      ...
      QH.ExAst (expr, patt)
    ;;
    List.iter (fun (q, f) -> QH.add q f)
      ["attribute_body",
       apply_entry attribute_body_eoi Meta_E.attribute_body
         Meta_P.attribute_body;
       "class_expr",
       ...
       "type_extension",
       apply_entry type_extension_eoi Meta_E.type_extension
         Meta_P.type_extension;
       "with_constr",
         ...
         [None, None,
          [Grammar.production
             (Grammar.r_next
                ...
                (Grammar.s_token ("EOI", "")),
              "194fe98d",
              (fun _ (e : 'Base__Parsers__patt) (loc : Ploc.t) ->
                 (let loc = Ploc.make_unlined (0, 0) in
                  if !(Pcaml.strict_mode) then
                    MLast.PaApp
                      (loc,
