    ...
    module Conv (C : ConvSig) =
      struct
        ...
        and expr =
          ...
          | ExBae (_, e, el) ->
              C.node "ExBae" [expr e; C.vala (C.list expr) el]
          ...
          | ExWhi (_, e, el) ->
              C.node "ExWhi" [expr e; C.vala (C.list expr) el]
        and module_type =
          function
            MtAcc (_, mt1, mt2) ->
        ...
        and sig_item =
          ...
          | SgMty (_, s, mt) ->
              C.node "SgMty" [C.vala C.string s; module_type mt]
    ...
    module P =
      Conv
        (struct
           ...
           let bool b =
             if b then MLast.PaUid (loc, "True")
             else MLast.PaUid (loc, "False")
           ;;
...
List.iter (fun (q, f) -> Quotation.add q f)
  ["expr", apply_entry expr_eoi Meta.E.expr Meta.P.expr;
   ...
   "class_str_item",
   apply_entry class_str_item_eoi Meta.E.class_str_item Meta.P.class_str_item;
   "class_sig_item",
