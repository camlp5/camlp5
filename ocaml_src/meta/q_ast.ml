...
module Meta_make (C : MetaSig) =
  struct
    ...
    and patt =
      ...
      | PaUnp (_, s, omt) ->
        let c_string_opt sopt = C.option C.string sopt in
          C.node "PaUnp" [C.vala c_string_opt s; C.option module_type omt]
    ...
    and expr =
      ...
      | ExLmd (_, s, me, e) ->
        let c_string_opt sopt = C.option C.string sopt in
          C.node "ExLmd" [C.vala c_string_opt s; module_expr me; expr e]
    ...
    and module_type =
      ...
      | MtFun (_, arg, mt2) ->
          let c_string_opt sopt = C.option C.string sopt in
          let c_tuple (sopt, mt) =
            C.tuple [c_string_opt sopt; module_type mt] in
          ...
          C.node "SgMod"
            [C.vala C.bool b;
             C.vala
               (C.list
                  (fun (sopt, mt) ->
                     C.tuple [C.vala c_string_opt sopt; module_type mt]))
               lsmt]
          ...
          C.node "StMod"
            [C.vala C.bool b;
             C.vala
               (C.list
                  (fun (sopt, me) ->
                     C.tuple [C.vala c_string_opt sopt; module_expr me]))
               lsme]
