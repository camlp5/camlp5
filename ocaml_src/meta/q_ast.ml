...
module Meta =
  struct
    ...
    and p_ctyp =
      ...
      | TyObj (_, lm, v) ->
          p_node "TyObj"
            [p_vala
               (p_list
                  (fun (s, t) -> MLast.PaTup (loc, [p_string s; p_ctyp t])))
               lm;
             p_vala p_bool v]
      | TyOlb (_, i, t) -> p_node "TyOlb" [p_vala p_string i; p_ctyp t]
      | TyPol (_, lv, t) ->
          p_node "TyPol" [p_vala (p_list p_string) lv; p_ctyp t]
            ...
            p_vala
              (p_list
                 (fun (loc, lab, mf, t) ->
                    MLast.PaTup
                      (loc,
                       [MLast.PaAny loc; MLast.PaStr (loc, lab); p_bool mf;
                        p_ctyp t])))
              lld
          in
          p_node "TyRec" [lld]
            ...
            p_vala
              (p_list
                 (fun (loc, lab, lt) ->
                    ...
                    MLast.PaTup
                      (loc, [MLast.PaAny loc; p_vala p_string lab; lt])))
              lcd
          in
          p_node "TySum" [lcd]
    ...
    let rec e_patt =
      ...
      | PaLab (_, s, p) ->
          e_node "PaLab" [e_vala e_string s; e_option e_patt p]
      ...
      | PaOlb (_, s, opeo) ->
          e_node "PaOlb"
            [e_vala e_string s;
             e_option
               (fun (p, oe) ->
                  MLast.ExTup (loc, [e_patt p; e_vala (e_option e_expr) oe]))
               opeo]
