...
module PA (Base : Mlsyntax.PARSEBASESIG) (QH : Quotation.QUOTATION_EXPANSION with module Base = Base) :
  ...
  struct
    ...
    open Base.Parsers;;
    open Base;;
    let odfa = !(Lexer.dollar_for_antiquotation) in
    let osrs = !(Lexer.simplest_raw_strings) in
    Lexer.dollar_for_antiquotation := false;
    Lexer.simplest_raw_strings := false;
    Lexer.utf8_lexing := true;
    Grammar.Unsafe.gram_reinit gram (Lexer.gmake ());
    Lexer.dollar_for_antiquotation := odfa;
    Lexer.simplest_raw_strings := osrs;
    ...
    let uv = Pcaml.unvala;;
    let vala_map = Pcaml.vala_map;;
    let vala_it = Pcaml.vala_it;;
    let mksequence2 loc =
      function
        [e] -> e
      ...
      let rec loop top =
        ...
        | e1 :: el ->
            ...
            MLast.ExApp
              (loc,
               MLast.ExApp
                 (loc, MLast.ExLong (loc, MLast.LiUid (loc, "::")), e1),
               loop false el)
      in
      loop true
    ;;
    ...
    let build_op_attributed loc op attrs =
      List.fold_left (fun e a -> MLast.ExAtt (loc, e, a))
        (MLast.ExLid (loc, op)) attrs
    ;;
      ...
      let (argpat, argexp) =
        ...
        List.fold_left
          (fun (argpat, argexp) (andop, (pat, exp)) ->
             ...
             MLast.ExApp
               (loc, MLast.ExApp (loc, MLast.ExLid (loc, andop), argexp),
                exp))
          b l
      in
      MLast.ExApp
        ...
        | Some ("ANTIQUOT_LOC", s)
          when
            match Plexing.parse_antiloc s with
              Some
                (_, ("list" | "_list" | "lid" | "_lid" | "flag" | "_flag"),
                 _) ->
                true
            | _ -> false ->
            wrec (n + 1)
    ...
    ;;
    let check_colon =
      Grammar.Entry.of_parser gram "check_colon" check_colon_f
    ;;
    ...
    let watch__locstr_expr (x : Ploc.t * string) (e : MLast.expr) = ();;
    ...
    Grammar.safe_extend
      (let _ = (sig_item : 'sig_item Grammar.Entry.e)
       ...
       in
       let located_rawstring : 'located_rawstring Grammar.Entry.e =
         grammar_entry_create "located_rawstring"
       and located_string : 'located_string Grammar.Entry.e =
         grammar_entry_create "located_string"
       ...
       and str_item_exception : 'str_item_exception Grammar.Entry.e =
         grammar_entry_create "str_item_exception"
       and str_item_module : 'str_item_module Grammar.Entry.e =
       ...
       in
       [Grammar.extension
          (located_rawstring : 'located_rawstring Grammar.Entry.e) None
          [None, None,
           [Grammar.production
              (Grammar.r_next Grammar.r_stop
               ...
               (fun (s : string) (loc : Ploc.t) ->
                  (let (delimsize, s) = Asttools.split_rawstring (uv s) in
                   let loc =
                     Asttools.narrow_loc loc (delimsize + 2) (delimsize + 2)
                   in
                   loc, s :
                   'located_rawstring)))]];
        Grammar.extension (located_string : 'located_string Grammar.Entry.e)
          None
          [None, None,
           [Grammar.production
              (Grammar.r_next Grammar.r_stop (Grammar.s_token ("STRING", "")),
               "194fe98d",
               (fun (s : string) (loc : Ploc.t) ->
                  (let loc = Asttools.narrow_loc loc 1 1 in loc, s :
                   'located_string)))]];
           ...
           [Grammar.production
              (Grammar.r_next Grammar.r_stop
               ...
               (fun (l : 'alg_attribute list) (loc : Ploc.t) ->
                  (l : 'alg_attributes)))]];
        Grammar.extension
          (alg_attributes_no_anti : 'alg_attributes_no_anti Grammar.Entry.e)
          None
          [None, None,
           [Grammar.production
              (Grammar.r_next Grammar.r_stop
            ...
            Grammar.production
              (Grammar.r_next
               ...
               (fun _ (ls : 'typevar list) _ (loc : Ploc.t) ->
                  (ls : 'type_binder_opt)))]];
        Grammar.extension
          (str_item_exception : 'str_item_exception Grammar.Entry.e) None
          [None, None,
           [Grammar.production
              (Grammar.r_next
                 (Grammar.r_next
               ...
               (fun (item_attrs : 'item_attributes)
                    (ec : 'extension_constructor) (ext : 'ext_opt)
                    (loc : Ploc.t) ->
                  (str_item_to_inline (MLast.StExc (loc, ec, item_attrs))
                     ext :
                   'str_item_exception)))]];
        Grammar.extension (str_item_module : 'str_item_module Grammar.Entry.e)
          None
          [None, None,
           [Grammar.production
              (Grammar.r_next
               ...
               "194fe98d",
               (fun (l : 'mod_binding list) (r : bool)
                    (ext, attrs1 : 'ext_attributes) (loc : Ploc.t) ->
                  (let l =
                     match l, attrs1 with
                       h :: t, _ :: _ ->
                         let (i, me, attrs) = h in
                         let attrs =
                           merge_left_auxiliary_attrs
                             ~nonterm_name:"str_item-module"
                             ~left_name:"algebraic attributes"
                             ~right_name:"item attributes" attrs1 attrs
                         in
                         let h = i, me, attrs in h :: t
                     | _, [] -> l
                     | _ -> failwith "str_item-module: syntax error"
                   in
                   str_item_to_inline (MLast.StMod (loc, r, l)) ext :
                   'str_item_module)))]];
        Grammar.extension (str_item_open : 'str_item_open Grammar.Entry.e)
          None
          [None, None,
           [Grammar.production
              (Grammar.r_next
               ...
               (fun (attrs : 'item_attributes) (me : 'module_expr)
                    (ext, attrs1 : 'ext_attributes) (ovf : bool)
                    (loc : Ploc.t) ->
                  (let attrs =
                     merge_left_auxiliary_attrs ~nonterm_name:"str_item-open"
                       ~left_name:"algebraic attributes"
                       ~right_name:"item attributes" attrs1 attrs
                   in
                   str_item_to_inline (MLast.StOpn (loc, ovf, me, attrs))
                     ext :
                   'str_item_open)))]];
        Grammar.extension
           ...
           [Grammar.production
              (Grammar.r_next
               ...
               (fun (attrs : 'item_attributes) (mt : 'module_type) _
                    (i : 'ident) (ext, alg_attrs : 'ext_attributes)
                    (loc : Ploc.t) ->
                  (let attrs =
                     merge_left_auxiliary_attrs
           ...
           [Grammar.production
              (Grammar.r_next
               ...
               (fun (te : 'type_extension) _ _ (loc : Ploc.t) ->
                  (MLast.StTypExten
                     (loc,
                      {MLast.teNam = te.MLast.teNam;
                       MLast.tePrm = te.MLast.tePrm;
                       MLast.tePrv = te.MLast.tePrv;
                       MLast.teECs = te.MLast.teECs;
            ...
            Grammar.production
              (Grammar.r_next
                 ...
                 (Grammar.s_list1sep
                    ...
                    (Grammar.s_token ("", "and")) false),
               "194fe98d",
               (fun (tdl : 'type_decl list) (nrfl : bool) _ _
                    (loc : Ploc.t) ->
                  (vala_it
                     (fun tdl ->
                        if List.exists (fun td -> not (uv td.MLast.tdIsDecl))
                             tdl
                        then
                          failwith
                            "type-declaration cannot mix decl and subst")
                     tdl;
           ...
           [Grammar.production
            ...
            Grammar.production
              (Grammar.r_next
                 ...
                 (Grammar.s_nterm
                    (item_attributes : 'item_attributes Grammar.Entry.e)),
               "194fe98d",
               (fun (attrs : 'item_attributes) (pd : string list) _
                    (t : 'ctyp) (ls : 'type_binder_opt) _
            ...
            Grammar.production
              (Grammar.r_next
               ...
               "194fe98d",
               (fun (attrs : 'item_attributes) (pd : string list) _
                    (t : 'ctyp) (ls : 'type_binder_opt) _ (i : string) _
                    (loc : Ploc.t) ->
                  (MLast.StExt (loc, i, ls, t, pd, attrs) : 'str_item)));
            Grammar.production
              (Grammar.r_next
           ...
           [Grammar.production
              (Grammar.r_next
               ...
               (fun (mt2 : 'module_type) _ (mt1 : 'module_type)
                    (loc : Ploc.t) ->
                  (MLast.MtFun (loc, Some (None, mt1), mt2) :
                   'module_type)))];
           Some "alg_attribute", Some Gramext.LeftA,
           [Grammar.production
              (Grammar.r_next
           ...
           [Grammar.production
            ...
            Grammar.production
              (Grammar.r_next
                 ...
                 (Grammar.s_token ("LIDENT", "")),
               "194fe98d",
               (fun (i : string) _ (li : 'extended_longident)
                    (loc : Ploc.t) ->
            ...
            Grammar.production
              (Grammar.r_next
                 ...
                 (Grammar.s_nterm
                    (item_attributes : 'item_attributes Grammar.Entry.e)),
               "194fe98d",
               (fun (attrs : 'item_attributes) (t : 'ctyp)
            ...
            Grammar.production
              (Grammar.r_next
               ...
               (fun (tdl : 'type_decl list) (nrfl : bool) _ _
                  ...
                  (vala_it
                     (fun tdl ->
                        if List.for_all (fun td -> uv td.MLast.tdIsDecl) tdl
                        then
                          ()
                        else if
                          List.for_all (fun td -> not (uv td.MLast.tdIsDecl))
                            tdl
                        then
                          vala_it
                            (fun nrfl ->
            ...
            Grammar.production
              (Grammar.r_next
                 ...
                 (Grammar.s_nterm
                    (item_attributes : 'item_attributes Grammar.Entry.e)),
               "194fe98d",
               (fun (attrs : 'item_attributes) (li : 'longident) _
           ...
           [Grammar.production
              (Grammar.r_next
                 ...
                 (Grammar.s_nterm
                    (module_type : 'module_type Grammar.Entry.e)),
               "194fe98d",
               (fun (mt : 'module_type) _ (li : 'longident) _ _
                    (loc : Ploc.t) ->
            ...
            Grammar.production
              (Grammar.r_next
                 ...
                 (Grammar.s_nterm
                    (module_expr : 'module_expr Grammar.Entry.e)),
               "194fe98d",
               (fun (me : 'module_expr) _ (li : 'longident) _
                    (loc : Ploc.t) ->
        ...
        Grammar.extension (andop_binding : 'andop_binding Grammar.Entry.e)
          None
          [None, None,
           [Grammar.production
              (Grammar.r_next
                       ...
                       [Grammar.production
                          (Grammar.r_next
                           ...
                           (fun (id : 'attribute_id) _ (loc : Ploc.t) ->
                              (id : 'e__10)))])),
               "194fe98d",
               (fun (ext : 'e__10 option) (loc : Ploc.t) ->
                  (ext : 'ext_opt)))]];
        Grammar.extension (ext_attributes : 'ext_attributes Grammar.Entry.e)
          None
          [None, None,
           [Grammar.production
              (Grammar.r_next
               ...
               "194fe98d",
               (fun (l : 'alg_attributes_no_anti) (e : 'ext_opt)
                    (loc : Ploc.t) ->
                  (e, l : 'ext_attributes)))]];
          ...
          [Some "top", Some Gramext.RightA,
           [Grammar.production
            ...
            Grammar.production
              (Grammar.r_next
               ...
               (fun _ (seq : 'sequence) _ _ (e2 : 'expr)
                    (df : 'direction_flag) (e1 : 'expr) _ (i : 'patt)
                    (ext, attrs : 'ext_attributes) _ (loc : Ploc.t) ->
            ...
            Grammar.production
              (Grammar.r_next
               ...
               (fun (l : 'closed_case_list) _ (e : 'expr)
                    (ext, attrs : 'ext_attributes) _ (loc : Ploc.t) ->
                  (expr_to_inline (MLast.ExTry (loc, e, l)) ext attrs :
                   'expr)));
            Grammar.production
              (Grammar.r_next
                 (Grammar.r_next
            ...
            Grammar.production
              (Grammar.r_next
               ...
               (fun (l : 'closed_case_list) _ (e : 'expr)
                    (ext, attrs : 'ext_attributes) _ (loc : Ploc.t) ->
                  (expr_to_inline (MLast.ExMat (loc, e, l)) ext attrs :
                   'expr)));
            Grammar.production
              (Grammar.r_next
                 (Grammar.r_next
               ...
               "194fe98d",
               (fun (e : 'fun_def) (p : 'ipatt) (ext, attrs : 'ext_attributes)
                    _ (loc : Ploc.t) ->
                  (expr_to_inline (MLast.ExFun (loc, [p, None, e])) ext
                     attrs :
                   'expr)));
            Grammar.production
              (Grammar.r_next
                 ...
                 (Grammar.r_next
                    (Grammar.r_next
                       (Grammar.r_next
                          (Grammar.r_next Grammar.r_stop
                             (Grammar.s_nterm
                                (letop : 'letop Grammar.Entry.e)))
                          (Grammar.s_nterm
                             (letop_binding :
                              'letop_binding Grammar.Entry.e)))
                       (Grammar.s_list0
                          (Grammar.s_nterm
                             (andop_binding :
                              'andop_binding Grammar.Entry.e))))
                    (Grammar.s_token ("", "in")))
                 (Grammar.s_nterml (expr : 'expr Grammar.Entry.e) "top"),
               "194fe98d",
               (fun (x : 'expr) _ (l : 'andop_binding list)
                    (b : 'letop_binding) (letop : 'letop) (loc : Ploc.t) ->
            ...
            Grammar.production
              (Grammar.r_next
                 ...
                 (Grammar.s_nterml (expr : 'expr Grammar.Entry.e) "top"),
               "194fe98d",
               (fun (x : 'expr) _ (l : 'let_binding list) (o : bool)
                  ...
                  (let l =
                     match l, attrs0 with
                       h :: t, _ :: _ ->
                         ...
                         let attrs =
                           merge_left_auxiliary_attrs
                             ~nonterm_name:"expr-let-rec"
                             ~left_name:"algebraic attributes"
                             ~right_name:"item attributes" attrs0 item_attrs
                         in
                         let h = a, b, attrs in h :: t
                     | _, [] -> l
                     | _ -> failwith "expr-let-rec: syntax error"
                   in
                   expr_to_inline (MLast.ExLet (loc, o, l, x)) ext0 [] :
                   'expr)));
            Grammar.production
              (Grammar.r_next
                 (Grammar.r_next
            ...
            Grammar.production
              (Grammar.r_next
                 (Grammar.r_next
                    (Grammar.r_next
                       (Grammar.r_next
                          ...
                          (Grammar.s_nterm
                             (item_extension :
                              'item_extension Grammar.Entry.e)))
                       (Grammar.s_nterm
                          (item_attributes :
                 ...
                 Grammar.s_self,
               "194fe98d",
               (fun (x : 'expr) _ (attrs1 : 'item_attributes)
                    (e : 'item_extension) (ext0, attrs0 : 'ext_attributes) _
                    (loc : Ploc.t) ->
                  (let si = MLast.StExten (loc, e, attrs1) in
                   let e = MLast.ExLSI (loc, si, x) in
                   expr_to_inline e ext0 attrs0 :
                   'expr)))];
           Some "where", None,
           [Grammar.production
              (Grammar.r_next
                 ...
                 (Grammar.s_nterm
                    (let_binding : 'let_binding Grammar.Entry.e)),
               "194fe98d",
               (fun (lb : 'let_binding) (rf : bool)
                    (ext, attrs : 'ext_attributes) _ (e : 'expr)
                    (loc : Ploc.t) ->
           ...
           [Grammar.production
              (Grammar.r_next
               ...
               (fun (e2 : 'expr) _ (e1 : 'expr) (loc : Ploc.t) ->
                  (MLast.ExApp
                     (loc, MLast.ExApp (loc, MLast.ExLid (loc, "||"), e1),
                      e2) :
                   'expr)))];
           Some "&&", Some Gramext.RightA,
           [Grammar.production
           ...
           [Grammar.production
              (Grammar.r_next
                 ...
                 (Grammar.s_token ("", "]")),
               "194fe98d",
               (fun _ (attr : 'attribute_body) _ (e1 : 'expr)
                    (loc : Ploc.t) ->
           ...
           [Grammar.production
            ...
            Grammar.production
              (Grammar.r_next
               ...
               (fun (e2 : 'expr) _ (e1 : 'expr) (loc : Ploc.t) ->
                  (MLast.ExApp
                     (loc, MLast.ExApp (loc, MLast.ExLid (loc, "mod"), e1),
                      e2) :
                   'expr)));
            Grammar.production
              (Grammar.r_next
               ...
               (fun (e2 : 'expr) _ (e1 : 'expr) (loc : Ploc.t) ->
                  (MLast.ExApp
                     (loc, MLast.ExApp (loc, MLast.ExLid (loc, "lxor"), e1),
                      e2) :
                   'expr)));
            Grammar.production
              (Grammar.r_next
               ...
               (fun (e2 : 'expr) _ (e1 : 'expr) (loc : Ploc.t) ->
                  (MLast.ExApp
                     (loc, MLast.ExApp (loc, MLast.ExLid (loc, "lor"), e1),
                      e2) :
                   'expr)));
            Grammar.production
              (Grammar.r_next
               ...
               (fun (e2 : 'expr) _ (e1 : 'expr) (loc : Ploc.t) ->
                  (MLast.ExApp
                     (loc, MLast.ExApp (loc, MLast.ExLid (loc, "land"), e1),
                      e2) :
                   'expr)));
            Grammar.production
              (Grammar.r_next
           ...
           [Grammar.production
            ...
            Grammar.production
              (Grammar.r_next
               ...
               (fun (e2 : 'expr) _ (e1 : 'expr) (loc : Ploc.t) ->
                  (MLast.ExApp
                     (loc, MLast.ExApp (loc, MLast.ExLid (loc, "lsr"), e1),
                      e2) :
                   'expr)));
            Grammar.production
              (Grammar.r_next
               ...
               (fun (e2 : 'expr) _ (e1 : 'expr) (loc : Ploc.t) ->
                  (MLast.ExApp
                     (loc, MLast.ExApp (loc, MLast.ExLid (loc, "lsl"), e1),
                      e2) :
                   'expr)));
            Grammar.production
              (Grammar.r_next
               ...
               (fun (e2 : 'expr) _ (e1 : 'expr) (loc : Ploc.t) ->
                  (MLast.ExApp
                     (loc, MLast.ExApp (loc, MLast.ExLid (loc, "asr"), e1),
                      e2) :
                   'expr)));
            Grammar.production
              (Grammar.r_next
           ...
           [Grammar.production
              (Grammar.r_next
                 ...
                 Grammar.s_self,
               "194fe98d",
               (fun (e : 'expr) (ext, attrs : 'ext_attributes) _
                    (loc : Ploc.t) ->
           ...
           [Grammar.production
            ...
            Grammar.production
              (Grammar.r_next
               ...
               "194fe98d",
               (fun (lili : 'longident_lident) _ (e1 : 'expr)
                    (loc : Ploc.t) ->
        ...
        Grammar.extension (expr_longident : 'expr_longident Grammar.Entry.e)
          None
          [None, None,
           [Grammar.production
              (Grammar.r_next
                 (Grammar.r_next
                    ...
                    (Grammar.s_nterm
                       (check_lbracketbar :
                        'check_lbracketbar Grammar.Entry.e)))
            ...
            Grammar.production
              (Grammar.r_next
               ...
               (fun _ (el : 'expr list) _ (e : 'expr) _ _ (li : 'longident)
                    (loc : Ploc.t) ->
                  (MLast.ExOpen (loc, li, mktupexp loc e el) :
                   'expr_longident)));
            Grammar.production
              (Grammar.r_next
                 (Grammar.r_next
            ...
            Grammar.production
              (Grammar.r_next
               ...
               (fun (op : 'operator_rparen) _ _ (li : 'longident)
                    (loc : Ploc.t) ->
                  (if op = "::" then
                     MLast.ExLong (loc, MLast.LiAcc (loc, li, op))
                   else
                     MLast.ExFle (loc, MLast.ExLong (loc, li), (None, op)) :
                   'expr_longident)));
            Grammar.production
              (Grammar.r_next Grammar.r_stop
              ...
              (Grammar.r_next
                 (Grammar.r_next
                    (Grammar.r_next
                       ...
                       (Grammar.s_nterm
                          (mod_fun_binding :
                           'mod_fun_binding Grammar.Entry.e)))
                    (Grammar.s_token ("", "in")))
                 Grammar.s_self,
               "194fe98d",
            ...
            Grammar.production
              (Grammar.r_next
               ...
               (fun (el : 'sequence) _ (l : 'let_binding list) (rf : bool) _
                    (loc : Ploc.t) ->
                  ([MLast.ExLet (loc, rf, l, mksequence loc el)] :
                   'sequence)))]];
              ...
              (Grammar.r_next
                 (Grammar.r_next
                    (Grammar.r_next
                       ...
                       (Grammar.s_nterm
                          (check_not_colon :
                           'check_not_colon Grammar.Entry.e)))
                    (Grammar.s_nterm
                       (fun_binding : 'fun_binding Grammar.Entry.e)))
                 (Grammar.s_nterm
                    (item_attributes : 'item_attributes Grammar.Entry.e)),
               "194fe98d",
               (fun (attrs : 'item_attributes) (e : 'fun_binding) _
                    (p : 'ipatt) (loc : Ploc.t) ->
              ...
              (Grammar.r_next
               ...
               (fun (attrs : 'item_attributes) (e : 'fun_binding) _
                  ...
                  (let (p, e) =
                   ...
                   p, e, attrs :
                   'let_binding)))]];
        Grammar.extension (letop_binding : 'letop_binding Grammar.Entry.e)
          None
          [None, None,
           [Grammar.production
              (Grammar.r_next
          ...
          [None, Some Gramext.LeftA,
           ...
           [Grammar.production
            ...
            Grammar.production
              (Grammar.r_next
                 ...
                 Grammar.s_self,
               "194fe98d",
               (fun (p : 'patt) (ext, attrs : 'ext_attributes) _
                    (loc : Ploc.t) ->
                  (patt_to_inline (MLast.PaExc (loc, p)) ext attrs :
                   'patt)))];
           None, Some Gramext.NonA,
           [Grammar.production
              (Grammar.r_next
           ...
           [Grammar.production
              (Grammar.r_next
               ...
               (fun (p : 'patt) (ext, attrs : 'ext_attributes) _
                    (loc : Ploc.t) ->
                  (patt_to_inline (MLast.PaLaz (loc, p)) ext attrs : 'patt)));
            Grammar.production
              (Grammar.r_next (Grammar.r_next Grammar.r_stop Grammar.s_self)
                 Grammar.s_self,
               "194fe98d",
           ...
           [Grammar.production
            ...
            Grammar.production
              (Grammar.r_next
               ...
               (fun (op : 'operator_rparen) _ (loc : Ploc.t) ->
                  (if op = "::" then
                     MLast.PaLong (loc, MLast.LiUid (loc, op), [])
           ...
           [Grammar.production
            ...
            Grammar.production
              (Grammar.r_next
               ...
               "194fe98d",
               (fun (clflag : 'e__12) _ (pl : 'patt list) (loc : Ploc.t) ->
                  (MLast.PaTup (loc, pl, clflag) : 'paren_patt)));
            Grammar.production
              (Grammar.r_next Grammar.r_stop
                 (Grammar.s_nterm (patt : 'patt Grammar.Entry.e)),
               "194fe98d",
          ...
          [Some "top", Some Gramext.LeftA,
           [Grammar.production
              (Grammar.r_next
                 ...
                 (Grammar.s_token ("", "]")),
               "194fe98d",
               (fun _ (attr : 'attribute_body) _ (e1 : 'ipatt)
                    (loc : Ploc.t) ->
            ...
            Grammar.production
              (Grammar.r_next
                 ...
                 (Grammar.s_rules
                    [Grammar.production
                       (Grammar.r_stop, "194fe98d",
                        (fun (loc : Ploc.t) -> (true : 'e__13)));
                     Grammar.production
                       (Grammar.r_next Grammar.r_stop
                          (Grammar.s_token ("", "..")),
                        "194fe98d",
                        (fun _ (loc : Ploc.t) -> (false : 'e__13)))]),
               "194fe98d",
               (fun (clflag : 'e__13) _ (pl : 'ipatt list) (loc : Ploc.t) ->
                  (MLast.PaTup (loc, pl, clflag) : 'paren_ipatt)));
            Grammar.production
              (Grammar.r_next Grammar.r_stop
                 (Grammar.s_nterm (ipatt : 'ipatt Grammar.Entry.e)),
               "194fe98d",
           ...
           [Grammar.production
              (Grammar.r_next
                 ...
                 (Grammar.s_nterm
                    (item_attributes : 'item_attributes Grammar.Entry.e)),
               "194fe98d",
               (fun (attrs : 'item_attributes) (cl : 'constrain list)
                    (tk : 'ctyp) (pf : bool) (isDecl : 'e__14)
                    (tpl : 'type_parameter list) (n : 'type_patt)
                    (loc : Ploc.t) ->
                  ({MLast.tdIsDecl = isDecl; MLast.tdNam = n;
                    MLast.tdPrm = tpl; MLast.tdPrv = pf; MLast.tdDef = tk;
                    MLast.tdCon = cl; MLast.tdAttributes = attrs} :
                   'type_decl)))]];
          ...
          [None, None,
           [Grammar.production
            ...
            Grammar.production
              (Grammar.r_next Grammar.r_stop (Grammar.s_token ("", "!-+")),
               "194fe98d",
               (fun _ (loc : Ploc.t) -> ("!-+" : 'type_variance)));
            Grammar.production
              (Grammar.r_next Grammar.r_stop (Grammar.s_token ("", "!+-")),
               "194fe98d",
               (fun _ (loc : Ploc.t) -> ("!+-" : 'type_variance)));
            ...
            Grammar.production
              (Grammar.r_next
                 ...
                 (Grammar.s_token ("", "-")),
               "194fe98d",
               (fun _ _ (loc : Ploc.t) -> ("!-" : 'type_variance)));
            Grammar.production
              (Grammar.r_next
                 (Grammar.r_next Grammar.r_stop (Grammar.s_token ("", "!")))
                 (Grammar.s_token ("", "+")),
           ...
           [Grammar.production
            ...
            Grammar.production
              (Grammar.r_next
                 ...
                 (Grammar.s_token ("UIDENT", "")),
               "194fe98d",
               (fun (i : string) _ _ (me1 : 'extended_longident)
                    (loc : Ploc.t) ->
           ...
           [Grammar.production
            ...
            Grammar.production
              (Grammar.r_next
                 ...
                 (Grammar.s_token ("LIDENT", "")),
               "194fe98d",
               (fun (i : string) _ (me1 : 'extended_longident)
                    (loc : Ploc.t) ->
          ...
          [Some "top", Some Gramext.LeftA,
           ...
           [Grammar.production
              (Grammar.r_next
                 ...
                 (Grammar.s_token ("", "]")),
               "194fe98d",
               (fun _ (attr : 'attribute_body) _ (e1 : 'ctyp)
                    (loc : Ploc.t) ->
           ...
           [Grammar.production
            ...
            Grammar.production
              (Grammar.r_next Grammar.r_stop
                 (Grammar.s_nterm (paren_ctyp : 'paren_ctyp Grammar.Entry.e)),
               "194fe98d",
               (fun (t : 'paren_ctyp) (loc : Ploc.t) -> (t : 'ctyp)));
            Grammar.production
              (Grammar.r_next Grammar.r_stop
                 (Grammar.s_nterm
                    (alg_extension : 'alg_extension Grammar.Entry.e)),
           ...
           [Grammar.production
            ...
            Grammar.production
              (Grammar.r_next
                 ...
                 (Grammar.s_token ("", ")")),
               "194fe98d",
               (fun _ (tl : 'maybe_labeled_ctyp list) _
                    (t : 'maybe_labeled_ctyp) _ (loc : Ploc.t) ->
          ...
          [None, None,
           [Grammar.production
              (Grammar.r_next
               ...
               "194fe98d",
               (fun (attrs : 'alg_attributes) (rto : 'e__16 option)
                    (loc : Ploc.t) ->
            ...
            Grammar.production
              (Grammar.r_next
                 ...
                 (Grammar.s_nterm
                    (alg_attributes : 'alg_attributes Grammar.Entry.e)),
               "194fe98d",
               (fun (attrs : 'alg_attributes) (rto : 'e__15 option)
                    (cal : 'ctyp_below_alg_attribute list)
              ...
              (Grammar.r_next
                 ...
                 (Grammar.s_nterm
                    (rest_constructor_declaration :
                     'rest_constructor_declaration Grammar.Entry.e)),
               "194fe98d",
               (fun (ls, tl, rto, alg_attrs : 'rest_constructor_declaration)
                    (ci : 'cons_ident) (attrs : 'alg_attributes_no_anti)
                    (loc : Ploc.t) ->
                  (let alg_attrs =
                     merge_left_auxiliary_attrs
                       ~nonterm_name:"extension_constructor"
                       ~left_name:"algebraic attributes"
                       ~right_name:"item attributes" attrs alg_attrs
                   in
                   MLast.EcTuple (loc, (loc, ci, ls, tl, rto, alg_attrs)) :
                   'extension_constructor)));
            Grammar.production
              (Grammar.r_next
                 (Grammar.r_next
           ...
           [Grammar.production
            ...
            Grammar.production
              (Grammar.r_next
                 ...
                 Grammar.s_self,
               "194fe98d",
               (fun (ce : 'class_expr) _ (lb : 'let_binding list) (rf : bool)
                    _ (loc : Ploc.t) ->
            ...
            Grammar.production
              (Grammar.r_next
               ...
               (fun (ce : 'class_fun_def) (p : 'ipatt)
                  ...
                  (class_expr_wrap_attrs (MLast.CeFun (loc, p, ce))
                     alg_attrs :
                   'class_expr)))];
           Some "alg_attribute", Some Gramext.LeftA,
           [Grammar.production
           ...
           [Grammar.production
              (Grammar.r_next Grammar.r_stop
               ...
               (fun (ce : 'class_expr_apply) (loc : Ploc.t) ->
                  (ce : 'class_expr)))]];
        Grammar.extension
          (class_expr_apply : 'class_expr_apply Grammar.Entry.e) None
          [Some "apply", Some Gramext.LeftA,
           [Grammar.production
              (Grammar.r_next (Grammar.r_next Grammar.r_stop Grammar.s_self)
           ...
           [Grammar.production
              (Grammar.r_next Grammar.r_stop
               ...
               (fun (ce : 'class_expr_simple) (loc : Ploc.t) ->
                  (ce : 'class_expr_apply)))]];
        Grammar.extension
          (class_expr_simple : 'class_expr_simple Grammar.Entry.e) None
          [Some "simple", None,
           [Grammar.production
              (Grammar.r_next
                 (Grammar.r_next
            ...
            Grammar.production
              (Grammar.r_next
               ...
               (fun (attrs : 'item_attributes) (e : 'fun_binding)
                    (topt : 'polyt option) (l : 'lident) (pf : bool)
                    (ovf : bool) _ (loc : Ploc.t) ->
            ...
            Grammar.production
              (Grammar.r_next
               ...
               (fun (attrs : 'item_attributes) (e : 'cvalue_binding)
                    (lab : 'lident) (mf : bool) (ovf : bool) _
                    (loc : Ploc.t) ->
                  (MLast.CrVal (loc, ovf, mf, lab, e, attrs) :
                   'class_str_item)));
            Grammar.production
              (Grammar.r_next
                 (Grammar.r_next
           ...
           [Grammar.production
            ...
            Grammar.production
              (Grammar.r_next
                 ...
                 (Grammar.s_token ("", "end")),
               "194fe98d",
               (fun _ (csf : 'e__19 list) (cst : 'class_self_type option) _
                    (loc : Ploc.t) ->
           ...
           [Grammar.production
            ...
            Grammar.production
              (Grammar.r_next
               ...
               (fun (attrs : 'item_attributes) (t : 'ctyp) _ (l : 'lident)
                  ...
                  (MLast.CgVal (loc, mf, vf, l, t, attrs) :
                   'class_sig_item)));
            Grammar.production
              (Grammar.r_next
                 (Grammar.r_next
            ...
            Grammar.production
              (Grammar.r_next
                 ...
                 (Grammar.s_token ("", "end")),
               "194fe98d",
               (fun _ (st : 'e__20 list) _ (loc : Ploc.t) ->
            ...
            Grammar.production
              (Grammar.r_next
               ...
               (fun (cli : 'longident_lident) (ext, attrs : 'ext_attributes) _
                    (loc : Ploc.t) ->
                  (expr_to_inline (MLast.ExNew (loc, cli)) ext attrs :
                   'expr)))]];
        Grammar.extension (expr : 'expr Grammar.Entry.e)
          (Some (Gramext.Level "."))
          [None, None,
           ...
           [Grammar.production
              (Grammar.r_next
               ...
               (fun (alg_attrs : 'alg_attributes)
                    (t : 'ctyp_below_alg_attribute) _ (loc : Ploc.t) ->
            ...
            Grammar.production
              (Grammar.r_next
               ...
               "194fe98d",
               (fun (alg_attrs : 'alg_attributes)
                    (t : 'ctyp_below_alg_attribute) _ (lab : string) _
                    (loc : Ploc.t) ->
           ...
           [Grammar.production
            ...
            Grammar.production
              (Grammar.r_next
                 ...
                 (Grammar.s_token ("", "}")),
               "194fe98d",
               (fun _ (eo : 'e__21 option) (p : 'patt_tcon) _ _
                    (loc : Ploc.t) ->
                       ...
                       [Grammar.production
                          (Grammar.r_next
                           ...
                           (fun (p : 'patt) _ (loc : Ploc.t) ->
                              (p : 'e__22)))])),
               "194fe98d",
               (fun (po : 'e__22 option) (p : 'patt_tcon) (loc : Ploc.t) ->
           ...
           [Grammar.production
            ...
            Grammar.production
              (Grammar.r_next
                 ...
                 (Grammar.s_token ("", "}")),
               "194fe98d",
               (fun _ (eo : 'e__23 option) (p : 'ipatt_tcon) _ _
                    (loc : Ploc.t) ->
                       ...
                       [Grammar.production
                          (Grammar.r_next
                           ...
                           (fun (p : 'patt) _ (loc : Ploc.t) ->
                              (p : 'e__24)))])),
               "194fe98d",
               (fun (po : 'e__24 option) (p : 'ipatt_tcon) (loc : Ploc.t) ->
           ...
           [Grammar.production
            ...
            Grammar.production
              (Grammar.r_next
                 ...
                 (Grammar.s_token ("", ")")),
               "194fe98d",
               (fun _ (e : 'expr) _ (j : string) _ (i : string)
                    (loc : Ploc.t) ->
                  (MLast.PaOlb
                     (loc, MLast.PaLid (loc, i),
                      Some
                        (MLast.ExOlb (loc, MLast.PaLid (loc, j), Some e))) :
                   'patt_option_label)));
            Grammar.production
              (Grammar.r_next
                 (Grammar.r_next
                 ...
                 (Grammar.s_token ("", ")")),
               "194fe98d",
               (fun _ (t : 'ctyp) _ (j : string) _ (i : string)
                    (loc : Ploc.t) ->
                  (MLast.PaOlb
                     (loc, MLast.PaTyc (loc, MLast.PaLid (loc, i), t),
              ...
              (Grammar.r_next
                 ...
                 (Grammar.s_opt
                    (Grammar.s_nterm
                       (fun_binding : 'fun_binding Grammar.Entry.e))),
               "194fe98d",
               (fun (eo : 'fun_binding option) (p : 'ipatt_tcon)
                    (loc : Ploc.t) ->
    ...
    Grammar.safe_extend
      (let _ = (interf : 'interf Grammar.Entry.e)
       ...
       [Grammar.extension (interf : 'interf Grammar.Entry.e) None
        ...
        Grammar.extension (sig_item_semi : 'sig_item_semi Grammar.Entry.e)
          None
          [None, None,
           [Grammar.production
              (Grammar.r_next
        ...
        Grammar.extension (str_item_semi : 'str_item_semi Grammar.Entry.e)
          None
          [None, None,
           [Grammar.production
              (Grammar.r_next
