...
parsing fails with an error message and the compilation is
...
where "directory" is the directory path where the Camlp5 library files
are installed.
...
with the extension ``.cmo`` or ``.cma``. They are the result of the
compilation of OCaml source files containing what is necessary to do
the parsing or printing. These object files are named parsing and
printing kits.
...
core, typically belonging to the module ``Pcaml``. The kits are designed
to be loaded by the Camlp5 commands, either through their command
arguments or through directives in the source files.

It is therefore important to compile the kits with the option ``-c`` of
the OCaml compiler (i.e. just compilation, not producing an
executable) and with the option ``-I +camlp5`` (or ``-I `camlp5 -where` ``)
...
understand. The same examples can be written in normal syntax. In case
of problems, refer to the chapter describing it.
