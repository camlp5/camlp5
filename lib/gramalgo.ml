...
(*
let nodes = Array.create (Array.length blr.item_set_tab) [] in
loop [] 0 where rec loop path j =
  if j = i then ()
  else if List.mem_assoc j path then ()
  else
    List.iter
      (fun (t, k) ->
         let path = [(j, t) :: path] in
         if List.mem path nodes.(k) then ()
         else do {
           nodes.(k) := [path :: nodes.(k)];
           loop path k
          })
      blr.term_shift_tab.(j)
;
List.iter
  (fun path -> do {
     Printf.eprintf "  path";
