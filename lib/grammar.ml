(* camlp5r *)
(* $Id: grammar.ml,v 1.77 2010/08/29 23:59:53 deraugla Exp $ *)
...
value iter_entry f e =
  ...
  and do_symbol =
    ...
    | Sopt s -> do_symbol s
    | Sflag s -> do_symbol s
    | Slist0sep s1 s2 -> do { do_symbol s1; do_symbol s2 }
    | Slist1sep s1 s2 -> do { do_symbol s1; do_symbol s2 }
    | Stree t -> do_tree t
...
value bparse_parsable entry p = do {
  ...
  struct
    ...
    value create g n =
      {egram = g; ename = n; elocal = False; estart = empty_entry n;
       econtinue _ _ _ = parser []; bstart _ = bparser [];
