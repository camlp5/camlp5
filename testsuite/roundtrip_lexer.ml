...
value is_version s = Pcre2.pmatch ~{pat="^OCAML_[0-9_]+(?:_[0-9a-zA-Z]+)$"} s ;
...
value setup_syntax () =
  ...
  Pcaml.Lexer.simplest_raw_strings.val := False;
  ...
  Pcaml.Lexer.utf8_lexing.val := True ;
  Pcaml.Lexer.no_quotations.val := True
    }
...
value lex_stream1 is = do {
  setup_syntax() ;
  let lexer = Pcaml.Lexer.gmake() in
...
value quot_re = Pcre2.(regexp ~{flags=[`DOTALL]} "^([^:@]*)([:@])(.*)$") ;

value nonws_re = Pcre2.(regexp ~{flags=[`DOTALL]} ".*\\S") ;
value nl_re = Pcre2.(regexp ~{flags=[`DOTALL]} ".*\n") ;
...
value pp_stream1 oc strm =
  let comment loc =
    ...
    if strip_comments.val && Pcre2.pmatch ~{rex=nonws_re} s then
      if Pcre2.pmatch ~{rex=nl_re} s then "\n" else " "
    else s in
  let rec pp1 =
    parser
      [
      ...
      | [: `(loc,("QUOTATION",qstring)) ; strm :] -> do {
          output_string oc (comment loc) ;
          let (qname, qbody) =
            try let strs = Pcre2.(extract ~{rex=quot_re} qstring) in
                if strs.(2) = "@" then
                  (strs.(1)^":", strs.(3))
                else (strs.(1), strs.(3))
