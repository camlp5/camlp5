...
|foo}
    ...
    {name="dot-parens-6"; implem = True ;
     exclude=["r2official";"o2official"];
     o_input = OK {foo|M.N.(x,y)|foo} ;
     official_input = OK {foo|M.N.(x,y)|foo} ;
     r_input = OK {foo|M.N.(x,y);|foo} ;
     o_output = OK {foo|let _ = M.N.(x,y);;
|foo};
     official_output = OK {foo|;;let open M.N in (x,y)|foo} ;
     r_output = OK {foo|M.N.(x,y);
|foo}
    };
    {(skip) with
     name="dot-parens-6-[ro]2official"; implem = True ;
     exclude=[];
     o_input = OK {foo|M.N.(x,y)|foo} ;
     r_input = OK {foo|M.N.(x,y);|foo} ;
     official_output = OK {foo|;;let open M.N in (x, y)|foo}
...
|foo}
    ...
    {name="anon-module-argument"; implem = True ;
     ...
     o_output = OK {foo|let f (module _ : A.B.S) = ();;
|foo};
     official_output = OK {foo|let f (module _ : A.B.S) = ()|foo} ;
     r_output = OK {foo|value f (module _ : A.B.S) = ();
|foo}
    }
    END
...
|foo}
    ...
    {name="named-module-argument"; implem = True ;
     ...
     o_output = OK {foo|let f (module M : A.B.S) = ();;
|foo};
     official_output = OK {foo|let f (module M : A.B.S) = ()|foo} ;
     r_output = OK {foo|value f (module M : A.B.S) = ();
|foo}
    }
    END
...
|foo}
    ...
    {name="str_item-module_type-1"; implem = True ;
     exclude=[];
     o_input = OK {foo|module type %e [@a] M = N [@@b]|foo} ;
     official_input = OK {foo|module type %e [@a] M = N [@@b]|foo} ;
     r_input = OK {foo|module type %e [@a] M = N [@@b];|foo} ;
     o_output = OK {foo|[%%e module type M = N[@@a] [@@b];;];;|foo};
     official_output = OK {foo|[%%e module type M  = N[@@a ][@@b ]]|foo} ;
     r_output = OK {foo|[%%"e" module type M = N[@@"a"] [@@"b"];];|foo}
    };
    {name="str_item-let-0"; implem = True ;
     exclude=[];
     o_input = OK {foo|let x = y in ()|foo} ;
     official_input = OK {foo|let x = y in ()|foo} ;
     r_input = OK {foo|let x = y in ();|foo} ;
     o_output = OK {foo|let _ = let x = y in ();;|foo};
     official_output = OK {foo|;;let x = y in ()|foo} ;
     r_output = OK {foo|let x = y in ();|foo}
    };
    {name="expr-let-0"; implem = True ;
     exclude=[];
     o_input = OK {foo|let x = let x = y in ()|foo} ;
     official_input = OK {foo|let x = let x = y in ()|foo} ;
     r_input = OK {foo|value x = let x = y in ();|foo} ;
     o_output = OK {foo|let x = let x = y in ();;|foo} ;
     official_output = OK {foo|let x = let x = y in ()|foo} ;
     r_output = OK {foo|value x = let x = y in ();|foo}
    };
...
;
    ...
    skip
    ELSE
    {name="str_item-let-module_type-0"; implem = True ;
     exclude=[];
     o_input = OK {foo|let module type A = M in ()|foo} ;
     official_input = OK {foo|let module type A = M in ()|foo} ;
     r_input = OK {foo|let module type A = M in ();|foo} ;
     o_output = OK {foo|let _ = let module type A = M in ();;|foo};
     official_output = OK {foo|;;let module type A = M in ()|foo} ;
     r_output = OK {foo|let module type A = M in ();|foo}
    }
    END
;
    IFDEF OCAML_VERSION < OCAML_5_5_0 THEN
...
;
    ...
    {name="expr-let-module_type-0"; implem = True ;
     ...
     official_input = OK {foo|let x = let module type A = M in ()|foo} ;
     r_input = OK {foo|value x = let module type A = M in ();|foo} ;
     o_output = OK {foo|let x = let module type A = M in ();;|foo} ;
     official_output = OK {foo|let x = let module type A = M in ()|foo} ;
     r_output = OK {foo|value x = let module type A = M in ();|foo}
    }
    END
;
    ...
    {name="expr-let-exception-0"; implem = True ;
     exclude=[];
